...
This is not a wrapper. imp implements its own GGUF parser, Jinja2 template engine, BPE tokenizer, paged KV cache, attention kernels (scalar, WMMA, CUTLASS FMHA), quantized GEMV (dp4a), MoE routing, Gated DeltaNet (GDN) recurrent layers, CUDA Graphs, and more — targeting Blackwell (sm_120) and Hopper (sm_90a) with CUDA 13.2.
...
- **Architectures:** LLaMA, Mistral, Mixtral, DeepSeek, Qwen3, Qwen3-MoE, **Qwen3.5 (Gated DeltaNet)**, Phi-4, Gemma-3 (text + vision), Nemotron-H (Mamba2 + Attention + MoE)
- **Model formats:** GGUF, SafeTensors, **MXFP4 GGUF** (native tensor-core weights)
- **Quantization:** Q2_K, Q3_K, Q4_0, Q4_K_M, Q5_K, Q6_K, Q8_0, FP8 E4M3, NVFP4 (FP4 E2M1), **MXFP4** (FP4 E2M1 + UE8M0), INT8
...
> **Tested models:** Qwen3-4B, Qwen3-8B, Qwen3-32B (Q4_K_M), Qwen3-Coder-30B (MoE), **Qwen3.5-4B, Qwen3.5-9B, Qwen3.5-27B, Qwen3.5-35B (GDN hybrid)**, Gemma-3-12B/27B, DeepSeek-R1-7B/14B, Nemotron-3-Nano-30B (Mamba2+MoE), Phi-4-Mini, **Llama-3.2-3B**, Llama 3.1 8B, Mistral 7B, Mixtral 8x7B, Devstral. Other models sharing the same architectures should work.
