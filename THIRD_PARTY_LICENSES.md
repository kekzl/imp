...
imp is MIT-licensed (`LICENSE`). The distribution (source tree, `imp-cli`,
`imp-server`, `libimp`, the `ghcr.io/kekzl/imp` image) also contains the code
below under its own licence. This file is the copy of each licence the
distribution has to carry and the attribution notices it has to reproduce.
`scripts/check-release.sh` fails a release without it, the image ships it at
`/usr/share/doc/imp/` and labels itself `MIT AND Apache-2.0`.
...
Attribution notice reproduced from the upstream source file (Apache-2.0
section 4(d)); the upstream repository has no NOTICE file:
...
The upstream repository's `LICENSE` follows verbatim; its appendix names the
repository's copyright holders.
...
```
   ...
   Copyright 2024 Jintao Zhang, Haofeng Huang
...
`third_party/stb/stb_image.h` and `third_party/stb/stb_image_resize2.h` are
