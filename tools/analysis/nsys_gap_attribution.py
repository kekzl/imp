...
def main():
    ...
    # Window is seconds from the first KERNEL, not the first interval of any kind: the weight
    # upload runs as memcpy for ~11s before the first kernel, so anchoring elsewhere shifts every
    # window and silently analyses the wrong part of the run.
    ...
    # Launch census: "fewer launches, not faster ones" (roadmap Open 1) needs the count per decode
    # step, a different ranking than time (a class cheap in time can be expensive in launches).
    # Steps are counted by the sampler kernel that ends one.
