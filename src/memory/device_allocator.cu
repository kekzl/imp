...
#define IMP_CUDA_CHECK(call)                                                                        \
    do {                                                                                            \
        ...
        if (err_ != cudaSuccess) {                                                                  \
            char buf[512];                                                                          \
            std::snprintf(buf, sizeof(buf), "CUDA error in %s at %s:%d – %s (%s)", #call, __FILE__, \
...
// Constructor
// ---------------------------------------------------------------------------
DeviceAllocator::DeviceAllocator(size_t initial_pool_size, int device) : device_(device) {
    ...
    uint64_t threshold = (initial_pool_size == 0) ? UINT64_MAX : static_cast<uint64_t>(initial_pool_size);
    IMP_CUDA_CHECK(cudaMemPoolSetAttribute(pool_, cudaMemPoolAttrReleaseThreshold, &threshold));
    ...
    int enable = 1;
    IMP_CUDA_CHECK(cudaMemPoolSetAttribute(pool_, cudaMemPoolReuseAllowOpportunistic, &enable));

    // Allow the pool to insert internal dependencies (events) between streams
    // so it can reuse memory across streams when safe.
    IMP_CUDA_CHECK(cudaMemPoolSetAttribute(pool_, cudaMemPoolReuseAllowInternalDependencies, &enable));
}
...
void* DeviceAllocator::allocate(size_t nbytes, cudaStream_t stream) {
    ...
    while (current > prev_peak &&
           !peak_allocated_.compare_exchange_weak(prev_peak, current, std::memory_order_relaxed)) {
...
// Queries
// ---------------------------------------------------------------------------
size_t DeviceAllocator::allocated() const { return allocated_.load(std::memory_order_relaxed); }

size_t DeviceAllocator::peak_allocated() const { return peak_allocated_.load(std::memory_order_relaxed); }

void DeviceAllocator::reset_peak_stats() {
    peak_allocated_.store(allocated_.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
