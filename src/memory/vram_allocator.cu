...
bool VRAMAllocator::init(float headroom_fraction) {
    ...
    if (err != cudaSuccess) {
        IMP_LOG_ERROR("VRAMAllocator: cudaMemGetInfo failed: %s",
    ...
    IMP_LOG_INFO("VRAMAllocator: total=%.0f MiB, headroom=%.0f MiB (%.0f%%)",
                 total_ / (1024.0 * 1024.0),
                 headroom_ / (1024.0 * 1024.0),
                 headroom_fraction * 100.0f);
    return true;
}
...
void* VRAMAllocator::allocate(size_t bytes, const char* tag) {
    ...
    if (err != cudaSuccess) {
        IMP_LOG_ERROR("VRAMAllocator: cudaMalloc failed for %s (%.2f MiB): %s",
                      tag, bytes / (1024.0 * 1024.0), cudaGetErrorString(err));
        return nullptr;
    }
...
void VRAMAllocator::report() const {
    ...
    IMP_LOG_INFO("VRAMAllocator report: tracked=%.0f MiB, free=%.0f MiB, "
    ...
    for (const auto& [tag, info] : by_tag) {
        IMP_LOG_INFO("  %-24s %8.1f MiB  (%d allocs)",
                     tag.c_str(),
                     info.first / (1024.0 * 1024.0),
                     info.second);
    }
}
