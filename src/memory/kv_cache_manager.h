...
public:
    ...
    // Allocate num_blocks new blocks for a sequence, appending to its existing block table.
    // On failure (pool exhausted mid-allocation) all blocks allocated during this call are
    // rolled back and the function returns false.
    ...
    // Free every block owned by a sequence (respecting ref-counts via cache_->free_block)
    // and remove it from all tracking structures. With prefix caching enabled, blocks whose
    // ref_count drops to 0 are kept in the block hash table for potential reuse.
    ...
    // Evict the least-recently-used sequence, freeing its blocks. Returns the evicted
    // seq_id, or -1. WARNING: every lru_order_ entry is a LIVE sequence and imp has no
    // recompute path, so the engine must NOT use this under KV pressure (it reject-newests
    // instead). Manager primitive only.
    int evict_lru();

    // Check whether num_blocks blocks are available: free pool plus reclaimable cached
    // blocks, minus what live sequences have already been promised (set_decode_reservation).
    // Used to also count live LRU sequences' blocks on the assumption evict_lru() could hand
    // them back; it cannot (no recompute path), so that counted memory that will never be
    // freed (#1635).
    [[nodiscard]] bool can_allocate(int num_blocks) const;

    // Promise total_blocks (prompt + decode) to a sequence at admission. The unwritten part
    // is subtracted from can_allocate() until the blocks are actually appended, so a later
    // request queues instead of being admitted against memory the running one needs (#1635).
    ...
    // result * kKVBlockSize tokens. Reuse stops at the first non-cached block (a hole after
    // an evicted block must not count). max_reuse_blocks caps reuse (-1 = unlimited);
    // hybrid models pass the recurrent-snapshot boundary so prefill never rewrites shared
    // blocks beyond the restorable state position. Returns -1 on allocation failure.
    // content_salt seeds the hash chain: the cache is addressed by TOKEN IDS alone, which is
    ...
    // hash of every FULL block of tokens (independent of cache state); used by the hybrid
    // recurrent-snapshot lookup to pick a restore boundary before allocation.
    ...
    // Pin the first num_blocks blocks of seq_id: never freed by evict_lru(),
    // reclaim_cached_block(), or free_sequence(). After free_sequence(), pinned blocks stay
    // in the cache pool with ref_count=1 for reuse. Re-pinning the same seq_id replaces its
    // previous pin set; when a pin budget is set, the oldest pin owners are unpinned (FIFO)
    // until the new pin fits, degrading their blocks to normal cached blocks.
    void pin_prefix(int seq_id, int num_blocks);

    // Remove pinning for all blocks pinned via pin_prefix() for the given sequence. Does NOT
    // free the blocks; they remain allocated as normal (or cached if the sequence was
    // already freed). Blocks shared with other pin owners stay pinned.
    ...
    // length/indexing as the global table, with -1 holes outside the trailing window.
    // Engine contract:
    //   - swa_prepare(seq, from, upto) BEFORE forwarding [from, upto): pads the SWA table to
    //     the global table's length, allocates live entries for blocks intersecting
    //     [from - window - slack, upto). Earlier slots stay/become -1.
    //   - swa_trim(seq, committed_tokens) after the step commits: frees entries fully below
    //     committed - window - slack. slack must cover the deepest spec-decode rollback.
    ...
    // SWA window snapshots (prefix caching for SWA-sized models): a snapshot is the packed
    // KV content of the live SWA window blocks at a block-aligned position P (for each SWA
    // layer, blocks [swa_first_live_block(P), P/bs)). Restoring it into a fresh sequence
    ...
    // Truncate a sequence's block table to fit new_seq_len tokens, freeing blocks beyond
    // what's needed. Caller must also truncate its own token vectors. Correctly handles
    // partial blocks (only frees blocks entirely past the new length).
    void rollback(int seq_id, int new_seq_len);

    // ── StreamingLLM middle eviction ─────────────────────────────────

    // Free the "middle" blocks of a sequence, keeping the first n_sink_tokens (attention
    // sinks) and the last n_window_tokens (sliding window). Sink blocks become pinned. Freed
    // slots become sentinel -1 so the block table length (positional alignment) is
    // unchanged; the decode kernel must be the streaming-aware variant (n_sinks > 0) so -1
    // slots are skipped. Idempotent once the sequence has been streamed. Returns the number
    // of physical blocks freed.
    ...
    // Save all cached (unreferenced) blocks to disk: KV data (GPU->host copy), hash
    // mappings, and metadata for validation. model_fingerprint identifies the producing
    // model/tokenizer/quant. Returns number of blocks saved, or -1 on error.
    ...
    // BitDecoding Phase 3: optional per-sequence ring buffer of FP16 K/V for the newest N
    // tokens. Decode kernel reads attention from BOTH the NVFP4 paged cache and the FP16
    // residual; the write path appends FP16 to the ring first and only quantizes to NVFP4
    // when the ring fills (evicting the overwritten entry to the paged cache).
    // Layout: [max_seqs, n_layers, 2 (K|V), residual_n, n_kv_heads, head_dim] FP16
    // contiguous, indexed by (seq_slot, layer, k_or_v).
    ...
    // not enabled, seq_id has no allocated slot, or layer is out of range.
    void* residual_k_ptr(int seq_id, int layer) const;
    void* residual_v_ptr(int seq_id, int layer) const;

    // Layer-base pointers (slot 0's K or V data for the given layer). Combined with
    // residual_seq_stride_bytes() a kernel addresses per-batch-idx data:
    // K_for_slot_s = K_layer_base + s * stride. Returns nullptr if residual not enabled or
    // layer out of range.
    ...
    // Device-resident ring state buffers ([max_seqs] ints each), replacing the per-step host
    // upload of write_idx/fill_count: kernels read directly, and
    // advance_residual_state_kernel updates them once per decode step (graph-capture-safe).
    // Zeroed on slot allocation and release.
...
private:
    // A sequence's positional block table. refs_[i] is the OWNING reference; an empty ref is
    // a hole, which StreamingLLM's evict_middle_blocks() creates deliberately (the table
    // LENGTH must not change, since attention kernels derive positional alignment from it).
    // block_table() still returns a plain vector<int> so the ~10 runtime/ consumers are
    // untouched; it's derived from refs_ and rebuilt lazily, so the two cannot drift.
    class SeqBlocks {
    public:
    ...
    // The cache's OWN reference to each entry, alongside its LRU position. Before A7 step
    // 3.2 the cache held nothing: free_sequence() deliberately skipped the free and the
    // block survived at refcount 1, liveness by omission. Erasing an entry now drops the
    // reference, which returns the block to the pool.
    struct CachedEntry {
    ...
    // where seq_slot in [0, residual_max_seqs_), layer in [0, n_layers), k_or_v in {0,1},
    // ring_idx in [0, residual_n_tokens_).
    ...
    // Slot allocator (multi-seq batch): free list of unused slot indices in
    // [0, residual_max_seqs_). Admission pops one off the back (LIFO for cache-
    // friendliness); release pushes back. seq_slot_map_ tracks the currently-assigned slot
    // per active seq_id.
