...
__global__ void dequant_int8_fp16_kernel(const int8_t* __restrict__ input, half* __restrict__ output,
                                         const half* __restrict__ scales, int n) {
    ...
    if (base + kElemsPerThread <= n) {
        ...
        half2 out01;
        out01 = __halves2half2(__float2half((float)v0 * s0), __float2half((float)v1 * s1));
        half2 out23;
        out23 = __halves2half2(__float2half((float)v2 * s2), __float2half((float)v3 * s3));
...
void dequant_int8_fp16(const void* input, void* output, const void* scales, int n, cudaStream_t stream) {
    ...
    dequant_int8_fp16_kernel<<<grid, kBlockSize, 0, stream>>>(static_cast<const int8_t*>(input),
