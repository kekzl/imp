#pragma once
// imp port of vLLM csrc/libtorch_stable/quantization/marlin/kernel.h
// (Apache-2.0, vLLM project / IST-DASLab Marlin). Declaration of the Marlin
// template kernel; explicit instantiations live in marlin_kernels_fp4_*.cu.
...
#define MARLIN_KERNEL_PARAMS                                                                                \
    const int4 *__restrict__ A, const int4 *__restrict__ B, int4 *__restrict__ C, int4 *__restrict__ C_tmp, \
        ...
        const int4 *__restrict__ zp_ptr, const int *__restrict__ g_idx, int num_groups, int prob_m,         \
        int prob_n, int prob_k, int lda, int *locks, bool has_bias, bool use_atomic_add,                    \
