...
// imp vendor shim (was: #include <torch/headeronly/util/Exception.h>):
// abort() is not constexpr, so a violated check inside a constexpr context
// becomes a compile error — same fail-loud behavior without torch.
#include <cstdlib>
