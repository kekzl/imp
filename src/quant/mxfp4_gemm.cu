...
__device__ __forceinline__ float warp_k_loop(
    const uint8_t* __restrict__ row_packed,
    const uint8_t* __restrict__ row_scales,
    int n_groups, int lane, DotFn dot_fn)
{
    float acc = 0.0f;
    ...
    int N, int K)
{
    ...
    float acc = gemv_mxfp4_row(
        packed_data + (int64_t)row * (K / 2),
        linear_scales + (int64_t)row * n_groups,
        x, n_groups, tid);
    ...
    int N, int K)
{
    ...
    if (row >= N || warp_id >= NR) return;

    float acc = warp_k_loop(
        packed_data + (int64_t)row * K_half,
        linear_scales + (int64_t)row * n_groups,
        n_groups, lane,
        [&] __device__ (const uint8_t* pb, int elem_base) {
...
template<int NR>
__global__ void __launch_bounds__(kMRThreads, 8)
gemv_mxfp4_multirow_fp32_kernel(
    const uint8_t* __restrict__ packed_data,
    const uint8_t* __restrict__ linear_scales,
    const half*    __restrict__ x,
    float*         __restrict__ y,
...
gemv_mxfp4_qkv_kernel(
    ...
    const uint8_t* __restrict__ v_packed, const uint8_t* __restrict__ v_scales,
    const half* __restrict__ x,
    ...
    int q_rows, int k_rows, int v_rows, int K)
{
    ...
    const uint8_t* packed; const uint8_t* scales; half* y_out; int row;
    if (global_row < q_rows) {
        row = global_row; packed = q_packed + (int64_t)row * K_half;
        scales = q_scales + (int64_t)row * n_groups; y_out = yq;
    } else if (global_row < q_rows + k_rows) {
        row = global_row - q_rows; packed = k_packed + (int64_t)row * K_half;
        scales = k_scales + (int64_t)row * n_groups; y_out = yk;
    } else if (global_row < q_rows + k_rows + v_rows) {
        row = global_row - q_rows - k_rows; packed = v_packed + (int64_t)row * K_half;
        scales = v_scales + (int64_t)row * n_groups; y_out = yv;
    } else { return; }

    float acc = gemv_mxfp4_row(packed, scales, x, n_groups, tid);
    float total = reduce_kpar(acc, tid, smem.warp_sums);
    if (tid == 0) y_out[row] = __float2half(total);
}
...
// ---------------------------------------------------------------------------
__global__ void __launch_bounds__(kKparThreads, 12)
gemv_mxfp4_gate_up_kernel(
    const uint8_t* __restrict__ g_packed, const uint8_t* __restrict__ g_scales,
    const uint8_t* __restrict__ u_packed, const uint8_t* __restrict__ u_scales,
    const half* __restrict__ x, half* __restrict__ yg, half* __restrict__ yu,
    int rows, int K)
{
    const int global_row = blockIdx.x;
    const int tid = threadIdx.x;
    ...
    const uint8_t* packed; const uint8_t* scales; half* y_out; int row;
    if (global_row < rows) {
        row = global_row; packed = g_packed + (int64_t)row * K_half;
        scales = g_scales + (int64_t)row * n_groups; y_out = yg;
    } else if (global_row < rows * 2) {
        row = global_row - rows; packed = u_packed + (int64_t)row * K_half;
        scales = u_scales + (int64_t)row * n_groups; y_out = yu;
    } else { return; }
...
__global__ void __launch_bounds__(kKparThreads, 12)
gemv_mxfp4_swiglu_residual_kernel(
    const uint8_t* __restrict__ packed_data,
    const uint8_t* __restrict__ linear_scales,
    const half*    __restrict__ gate,
    const half*    __restrict__ up,
...
__global__ void __launch_bounds__(kKparThreads, 12)
gemv_mxfp4_geglu_residual_kernel(
    const uint8_t* __restrict__ packed_data,
    const uint8_t* __restrict__ linear_scales,
    const half*    __restrict__ gate,
    const half*    __restrict__ up,
...
void gemv_mxfp4_kpar(const CutlassMxFP4Weight& W, const half* x, half* y,
                     int N, int K, cudaStream_t stream)
{
    ...
    if (++kpar_dbg <= 200 && (K != 5120 || kpar_dbg <= 3))
        fprintf(stderr, "[GEMV_DBG] N=%d K=%d W.N=%lld W.K=%lld data=%p scales=%p x=%p y=%p\n",
                N, K, (long long)W.N, (long long)W.K, W.data, W.linear_scales, x, y);
    int n_groups = K / kMxGroupSize;
    int mr_blocks = (N + kMRWarps - 1) / kMRWarps;
    if (use_multirow(n_groups, mr_blocks)) {
        gemv_mxfp4_multirow_kernel<kMRWarps><<<mr_blocks, kMRThreads, 0, stream>>>(
            static_cast<const uint8_t*>(W.data),
            static_cast<const uint8_t*>(W.linear_scales),
            x, y, N, K);
    } else {
        gemv_mxfp4_kpar_kernel<<<N, kKparThreads, 0, stream>>>(
            static_cast<const uint8_t*>(W.data),
            static_cast<const uint8_t*>(W.linear_scales),
            x, y, N, K);
    }
...
void gemv_mxfp4_kpar_fp32(const CutlassMxFP4Weight& W, const half* x, float* y,
                           int N, int K, cudaStream_t stream)
{
    int n_groups = K / kMxGroupSize;
    int mr_blocks = (N + kMRWarps - 1) / kMRWarps;
    if (use_multirow(n_groups, mr_blocks)) {
        gemv_mxfp4_multirow_fp32_kernel<kMRWarps><<<mr_blocks, kMRThreads, 0, stream>>>(
            static_cast<const uint8_t*>(W.data),
            static_cast<const uint8_t*>(W.linear_scales),
            x, y, N, K);
    } else {
...
void gemv_mxfp4_qkv_fused(const CutlassMxFP4Weight& wq, const CutlassMxFP4Weight& wk,
                           const CutlassMxFP4Weight& wv, const half* x,
                           ...
                           cudaStream_t stream)
{
    ...
    gemv_mxfp4_qkv_kernel<<<total_rows, kKparThreads, 0, stream>>>(
        ...
        static_cast<const uint8_t*>(wv.data), static_cast<const uint8_t*>(wv.linear_scales),
        x, yq, yk, yv, q_rows, k_rows, v_rows, K);
}

void gemv_mxfp4_gate_up_fused(const CutlassMxFP4Weight& wg, const CutlassMxFP4Weight& wu,
                               ...
                               int rows, int K, cudaStream_t stream)
{
    gemv_mxfp4_gate_up_kernel<<<rows * 2, kKparThreads, 0, stream>>>(
        ...
        static_cast<const uint8_t*>(wu.data), static_cast<const uint8_t*>(wu.linear_scales),
        x, yg, yu, rows, K);
}

void gemv_mxfp4_residual(const CutlassMxFP4Weight& W, const half* x, half* y,
                          const half* residual, int N, int K, cudaStream_t stream)
{
    gemv_mxfp4_residual_kernel<<<N, kKparThreads, 0, stream>>>(
        static_cast<const uint8_t*>(W.data),
        static_cast<const uint8_t*>(W.linear_scales),
        x, y, residual, N, K);
}

void gemv_mxfp4_swiglu_residual(const CutlassMxFP4Weight& W,
                                 const half* gate, const half* up,
                                 half* y, const half* residual,
                                 int N, int K, cudaStream_t stream)
{
    gemv_mxfp4_swiglu_residual_kernel<<<N, kKparThreads, 0, stream>>>(
        static_cast<const uint8_t*>(W.data),
        static_cast<const uint8_t*>(W.linear_scales),
        gate, up, y, residual, N, K);
}

void gemv_mxfp4_geglu_residual(const CutlassMxFP4Weight& W,
                                const half* gate, const half* up,
                                half* y, const half* residual,
                                int N, int K, cudaStream_t stream)
{
    gemv_mxfp4_geglu_residual_kernel<<<N, kKparThreads, 0, stream>>>(
        static_cast<const uint8_t*>(W.data),
        static_cast<const uint8_t*>(W.linear_scales),
        gate, up, y, residual, N, K);
}
