...
bool device_args_done = false;
{
    ...
    if (use_device_args) {
        ...
        // Tile height: cover the average expert in one pass (a 32-row tile per
        // CTA re-streamed the weights per tile and lost to the routing skew).
        const int rows_cfg = runtime_config().moe.nvfp4_prefill_smallm_v2_rows;
        const int avg_rows = expanded / std::max(ne, 1);
        const int v2_mt = rows_cfg == 32 || rows_cfg == 64 || rows_cfg == 128 ? rows_cfg
                          : avg_rows <= 24                                    ? 64
                                                                                : 128;
