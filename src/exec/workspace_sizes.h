...
// Exact, pre-upload demand for the engine-persistent (T2) tenants whose size is a pure
// function of model shape + engine config (MEMORY.md B5.1, A7 step 4b/6). The T2 arena
// acquires its Region when OPENED, so sizing it correctly reserves those bytes against
// everything that allocates later, including the pre-dequant cache build which expands
// into whatever free VRAM it finds (AUDIT B23). Pure and CUDA-free for CPU-lane testing;
// covers only the two tenants whose formulas are exact today, the arena keeps a floor
// for the rest.
...
// The shape facts the two functions below actually consume, extracted for CPU-lane
// testability. A wrong number here is the #1103 failure class (under-reservation),
// which is not something to leave to review.
struct ExecShape {
    ...
    // Shape inputs for the FP8 activation reduction scratch. Mirrors the max_dim ladder in
    // executor_workspace_buffers.cu, replicated rather than approximated: a demand function
    // that merely resembles the site is a plan that fits by luck.
    ...
    int ssm_dt_rank = 0;
    // Columns the Qwen3.5-style attention output gate writes. The gate half of a fused
    // q_proj is split into a buffer BORROWED from the SSM z buffer, so the two sizings are
    // coupled. 0 when the model has no gate (no layer's q_proj wider than n_heads*head_dim).
    ...
    // Raw facts for the dp4a input-staging cluster (q8_1+d8, FFN block mask, Q4_K/Q5_K
    // prefill pair). Deliberately NOT derived from `weights`: the site scans a narrower list
    // and reads shape[1]/shape[2] raw (no NVFP4 doubling, no output_proj, no gdn_gate), so
    // reusing exec_max_weight_k() would size from a K the kernels never see.
    // IMMA prefill activation scratch: TWO maxima kept apart on purpose. The dense route
    // runs at max_tokens rows against a dense K; the MoE route at max_tokens*top_k rows
    // against an expert K; the two never co-occur, and multiplying rows_max by K_max would
    // over-charge by a large factor.
    ...
    // The RESOLVED KV block size (Engine::init_resolve_kv_block_size_), not kKVBlockSize:
    // the two differ on n_kv_heads<=4 models and under kv_cache.block_size, and the split-K
    // partials are sized in blocks (AUDIT B-7). 0 = 16.
    int kv_block_size = 0;
};
...
// mmq_q8_imma.cu's split-K partial slices: a BOUND, not a measurement. The path needs
// M*N*used floats, runs only when M<=32, and its tile guard (n_tiles*S < 256, kBN=128)
// caps N*used at 512*128, so the buffer cannot exceed 32*65536*4 B.
...
struct ExecT2Demand {
    ...
    // The cuBLASLt workspace and algo-selection bench scratch gemm_init() takes: a fixed
    // 96 MiB, independent of the model, since cuBLASLt's workspace is a ceiling offered to
    // its heuristic, not a shape-derived size. Charged unconditionally: gemm_init() runs for
    // every model, encoder-only included.
    size_t cublas_workspace = 0;
    // CUTLASS 3.x grouped-GEMM staging + workspace, taken by
    // gemm_grouped_3x_nvfp4_prewarm(). MoE models only: engine.cpp gates the prewarm on
    // profile().is_moe, and a model without experts cannot reach the grouped path at all.
    size_t grouped3x = 0;
    ...
    // Small-M NVFP4 GEMM split-K workspace + packed 32-row activation scratch: the largest
    // per-weight workspace over the dense projections plus the activation for the widest K.
    // Charged for every model: which weights take the route is known only after
...
// max_tokens as GraphExecutor computes it: min(max_seq_len, 4096), then capped to 2048
// for SSM+MoE hybrids. Replicates the AS-BUILT condition, which reads has_gdn_ before
// it's assigned and fires only for SSM+MoE, never pure GDN (AUDIT B18); replicating the
// intended condition instead would under-reserve the arena by 2x.
...
// The batch, the FP8 prefill decision, and attention.mla_absorb are engine/runtime-
// config facts, not model facts, and three tenants are sized from them; the caller
...
// every hybrid staged today the two are exactly equal, so the borrow fits by arithmetic
// coincidence rather than construction; a checkpoint with a narrower recurrent inner
// size than n_heads*head_dim would overrun it silently, only in prefill.
...
// The (rows, K) pair the imma_scratch charge was computed from, the larger of the dense
// and MoE routes. engine.cpp hands it to mmq_q8_imma_preallocate() so the scratch is
// taken ONCE at the charged size; growing it incrementally would strand every
// intermediate slice in the bump arena.
