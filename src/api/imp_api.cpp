...
ImpError imp_gpu_release(int device_reset) {
    ...
    imp::trim_device_mempool();
    if (device_reset) {
        // Free + null lazily-created module-static CUDA resources (cuBLAS
        // handles, device scratch buffers) BEFORE the reset — otherwise their
        // pointers dangle with armed `if (!ptr)` guards and the next use after
        // an in-process reload touches freed device memory.
