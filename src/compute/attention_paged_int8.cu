...
__global__ void paged_attention_splitk_int8_kernel(
    ...
    const half* __restrict__ V_scales, float* __restrict__ partial_out, const int* __restrict__ block_tables,
...
#pragma unroll
    for (int i = 0; i < ELEMS; i++)
        q_amax = fmaxf(q_amax, fabsf(q_reg[i]));
...
void paged_attention_decode_int8(const Tensor& Q, const Tensor& K_cache, const Tensor& V_cache, Tensor& O,
    ...
    // gpt-oss) wired since #1345 (same as FP8, #1346).
    if (n_sinks > 0)
        sliding_window = 0;
...
#define LAUNCH_SPLITK_INT8(HD)                                                                             \
    paged_attention_splitk_int8_kernel<HD>                                                                 \
    ...
    IMP_CUDA_CHECK_LAUNCH()
        ...
        switch (head_dim) {
            case 64:
                LAUNCH_SPLITK_INT8(64);
                break;
            case 96:
                LAUNCH_SPLITK_INT8(96);
                break;
            case 128:
                LAUNCH_SPLITK_INT8(128);
                break;
            case 256:
...
#define LAUNCH_INT8_FALLBACK(HD)                                                                            \
    ...
    IMP_CUDA_CHECK_LAUNCH()
        ...
        switch (head_dim) {
            case 64:
                LAUNCH_INT8_FALLBACK(64);
                break;
            case 96:
                LAUNCH_INT8_FALLBACK(96);
                break;
            case 128:
                LAUNCH_INT8_FALLBACK(128);
                break;
            case 256:
