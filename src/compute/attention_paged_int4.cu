...
#pragma unroll
    ...
    crosswarp_reduce_splitk<HEAD_DIM>(reinterpret_cast<float*>(smem_sk_int4), m_w, l_w, o_reg, warp_id,
...
__global__ void paged_attention_splitk_int4_pipeline_kernel(
    const half* __restrict__ Q, const uint8_t* __restrict__ K_cache, const uint8_t* __restrict__ V_cache,
    const half* __restrict__ K_scales, const half* __restrict__ V_scales, float* __restrict__ partial_out,
    const int* __restrict__ block_tables, const int* __restrict__ context_lens, int batch_size, int n_heads,
    int n_kv_heads, int block_size, float scale, int max_num_blocks, int num_splits, int sliding_window,
    float softcap) {
    static_assert(HEAD_DIM % WARP_SIZE == 0, "HEAD_DIM must be divisible by WARP_SIZE");
...
#define LAUNCH_SPLITK_INT4_PIPE(HD)                                                                         \
    paged_attention_splitk_int4_pipeline_kernel<HD>                                                         \
    ...
    IMP_CUDA_CHECK_LAUNCH()
            ...
            switch (head_dim) {
                case 64:
                    LAUNCH_SPLITK_INT4_PIPE(64);
                    break;
                case 96:
                    LAUNCH_SPLITK_INT4_PIPE(96);
                    break;
                case 128:
                    LAUNCH_SPLITK_INT4_PIPE(128);
                    break;
                case 256:
...
#define LAUNCH_SPLITK_INT4(HD)                                                                             \
    paged_attention_splitk_int4_kernel<HD>                                                                 \
    ...
    IMP_CUDA_CHECK_LAUNCH()
            ...
            switch (head_dim) {
                case 64:
                    LAUNCH_SPLITK_INT4(64);
                    break;
                case 96:
                    LAUNCH_SPLITK_INT4(96);
                    break;
                case 128:
                    LAUNCH_SPLITK_INT4(128);
                    break;
                case 256:
