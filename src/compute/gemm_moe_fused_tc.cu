...
__global__ void __launch_bounds__(TC_BLOCK) gemm_q6k_fused_moe_prefill_tc_kernel(
    ...
    const int32_t* __restrict__ sorted_token_ids, int N, int K, size_t expert_stride_bytes, int n_experts,
    ...
    while (true) {
        if (tid == 0)
            s_tile_idx = atomicAdd(d_tile_counter, 1);
        __syncthreads();

        if (s_tile_idx >= total_tiles)
            return;
        ...
        while (lo < hi) {
            int mid = (lo + hi) >> 1;
            if (tile_prefix[mid + 1] <= m_tile_flat)
                lo = mid + 1;
            else
                hi = mid;
        }
...
#pragma unroll
                    for (int i = 0; i < 8; i++)
                        w_vals[i] = __float2half(0.0f);
                }

                *reinterpret_cast<uint4*>(&B_smem[row * TC_STRIDE + base_elem]) = *reinterpret_cast<uint4*>(
                    w_vals);
            }
...
#pragma unroll
            for (int kk = 0; kk < TC_K_TILE / 16; kk++) {
                wmma::load_matrix_sync(a_frag, &AC_smem[warp_m * 16 * TC_STRIDE + kk * 16], TC_STRIDE);
                wmma::load_matrix_sync(b_frag, &B_smem[warp_n * 16 * TC_STRIDE + kk * 16], TC_STRIDE);
        ...
        wmma::store_matrix_sync(&C_smem_f32[(warp_m * 16) * TC_TILE_N + (warp_n * 16)], c_frag, TC_TILE_N,
                                wmma::mem_row_major);
        ...
        for (int flat = tid; flat < TC_TILE_M * TC_TILE_N; flat += TC_BLOCK) {
            ...
            if (m < M_cur && n_global < N) {
                ...
                output[token * N + n_global] = __float2half(C_smem_f32[m * TC_TILE_N + n]);
            }
        }
