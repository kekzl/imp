...
//   2. QJL correction: qk_qjl = ||q|| * ||k|| * (2*popcount(XNOR(sketch_q, sketch_k)) - sketch_dim) /
//   sketch_dim
...
__global__ void __launch_bounds__(256, 2) paged_attention_decode_turboquant_kernel(
    ...
    const uint8_t* __restrict__ K_sketches,   // [total_blocks, block_size, n_kv_heads, sketch_dim/8] packed
                                              // bits
    const uint8_t* __restrict__ qjl_matrix,   // [sketch_dim, head_dim/8] packed Rademacher signs
    const uint8_t* __restrict__ K_mscales,    // MXFP4: UE8M0 micro-scales [blocks, block_size, n_kv_heads,
                                              // hd/32] (nullptr if !USE_MXFP4)
    half* __restrict__ O, const int* __restrict__ block_tables, const int* __restrict__ context_lens,
    int batch_size, int n_heads, int n_kv_heads, int block_size, float scale, int sketch_dim,
    int max_context_len, int max_num_blocks, int sliding_window, float softcap) {
...
#pragma unroll
    ...
    for (int blk = first_block + warp_id; blk < num_ctx_blocks; blk += NUM_WARPS) {
        ...
        const uint8_t* K_ms_block = USE_MXFP4 ? (K_mscales + (int64_t)phys_block * mscale_block_stride_v)
                                              : nullptr;
...
__global__ void __launch_bounds__(256, 2) paged_attention_splitk_turboquant_kernel(
    ...
    const uint8_t* __restrict__ K_sketches, const uint8_t* __restrict__ qjl_matrix,
    ...
    float scale, int sketch_dim, int max_num_blocks, int num_splits, int sliding_window, float softcap) {
    static_assert(HEAD_DIM % WARP_SIZE == 0);
    constexpr int ELEMS = HEAD_DIM / WARP_SIZE;
...
#pragma unroll
    ...
    for (int blk = split_start + warp_id; blk < split_end; blk += NUM_WARPS) {
        ...
        const half* V_sc_block = V_scales + (int64_t)phys_block * scale_block_stride;
        const uint8_t* K_sk_block = K_sketches + (int64_t)phys_block * sketch_kv_block_stride;
        const uint8_t* K_ms_block_sk = USE_MXFP4 ? (K_mscales + (int64_t)phys_block * mscale_block_stride_sk)
                                                 : nullptr;
...
#pragma unroll
    ...
    crosswarp_reduce_splitk<HEAD_DIM>(smem_red_base, m_w, l_w, o_reg, warp_id, lane_id, lane_offset,
                                      partial_out, batch_idx, n_heads, head_idx, num_splits, split_idx);
}

// ---------------------------------------------------------------------------
// Host launcher -- TurboQuant variant (with Split-K support)
// ---------------------------------------------------------------------------
void paged_attention_decode_turboquant(const Tensor& Q, const Tensor& K_dir_cache, const Tensor& V_cache,
                                       Tensor& O, const half* K_norms, const half* V_scales,
                                       const uint8_t* K_sketches, const uint8_t* qjl_matrix,
                                       const int* block_tables, const int* context_lens, int block_size,
                                       float scale, int sketch_dim, int max_context_len, int sliding_window,
                                       float softcap, cudaStream_t stream, int max_blocks_per_seq,
                                       const uint8_t* K_mscales, int n_sinks) {
    ...
    size_t smem_bytes = sketch_aligned + NUM_WARPS * sizeof(float) + NUM_WARPS * sizeof(float) +
...
__global__ void __launch_bounds__(256, 2) paged_attention_decode_turboquant_lite_kernel(
    const half* __restrict__ Q,              // [batch, n_heads, HEAD_DIM]
    const uint8_t* __restrict__ V_cache,     // INT4 packed values
    const half* __restrict__ K_norms,        // [total_blocks, block_size, n_kv_heads] FP16 norms
    const half* __restrict__ V_scales,       // [total_blocks, block_size, n_kv_heads] FP16 scales
...
#pragma unroll
    ...
    for (int blk = first_block + warp_id; blk < num_ctx_blocks; blk += NUM_WARPS) {
        ...
        for (int t = first_tok; t < (tok_end - tok_start); t++) {
            ...
            if (t + 1 < (tok_end - tok_start)) {
                const uint8_t* K_sk_next = K_sk_block + (t + 1) * sketch_slot_stride +
                                           kv_head * sketch_head_bytes;
                const uint8_t* V_next = V_block + (t + 1) * kv_slot_stride + kv_head * kv_head_bytes;
                if (lane_id == 0) {
                    asm volatile("prefetch.global.L1 [%0];\n" ::"l"(K_sk_next));
                    asm volatile("prefetch.global.L1 [%0];\n" ::"l"(V_next));
...
template <int HEAD_DIM>
__global__ void __launch_bounds__(256, 2) paged_attention_splitk_turboquant_lite_kernel(
    const half* __restrict__ Q, const uint8_t* __restrict__ V_cache, const half* __restrict__ K_norms,
    const half* __restrict__ V_scales, const uint8_t* __restrict__ K_sketches,
    const uint8_t* __restrict__ qjl_matrix, float* __restrict__ partial_out,
    const int* __restrict__ block_tables, const int* __restrict__ context_lens, int batch_size, int n_heads,
    int n_kv_heads, int block_size, float scale, int sketch_dim, int max_num_blocks, int num_splits,
...
// ---------------------------------------------------------------------------
void paged_attention_decode_turboquant_lite(const Tensor& Q, const Tensor& V_cache, Tensor& O,
                                            const half* K_norms, const half* V_scales,
                                            const uint8_t* K_sketches, const uint8_t* qjl_matrix,
                                            const int* block_tables, const int* context_lens, int block_size,
                                            float scale, int sketch_dim, int max_context_len,
...
#define LAUNCH_SPLITK_TQL(HD)                                                                           \
    paged_attention_splitk_turboquant_lite_kernel<HD><<<grid1, block1, smem_bytes, stream>>>(           \
        reinterpret_cast<const half*>(Q.data), reinterpret_cast<const uint8_t*>(V_cache.data), K_norms, \
        V_scales, K_sketches, qjl_matrix, partial, block_tables, context_lens, batch_size, n_heads,     \
        n_kv_heads, block_size, scale, sketch_dim, max_num_blocks, num_splits, sliding_window, softcap)

        switch (head_dim) {
            case 64:
                LAUNCH_SPLITK_TQL(64);
                break;
            case 96:
                LAUNCH_SPLITK_TQL(96);
                break;
            case 128:
                LAUNCH_SPLITK_TQL(128);
                break;
            case 256:
                LAUNCH_SPLITK_TQL(256);
                break;
            default:
...
#define LAUNCH_TQL(HD)                                                                                   \
    paged_attention_decode_turboquant_lite_kernel<HD><<<grid, block, smem_bytes, stream>>>(              \
        reinterpret_cast<const half*>(Q.data), reinterpret_cast<const uint8_t*>(V_cache.data), K_norms,  \
        V_scales, K_sketches, qjl_matrix, reinterpret_cast<half*>(O.data), block_tables, context_lens,   \
        batch_size, n_heads, n_kv_heads, block_size, scale, sketch_dim, max_context_len, max_num_blocks, \
