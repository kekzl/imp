...
struct Request {
    int id = 0;
    // The id the CALLER knows this request by (the server's `imp-N`), also what the HTTP
    // response carries; `id` above is the engine's own counter and the two never met (#1582).
    // Empty for direct API users; add_request() emits the join line when set.
    std::string trace_id;
    ...
    // Sticky acceptance verdict: structured-but-mutating content (number tables) re-trips the
    // acceptance economics after every re-arm window; once doomed, give-up is final for this request.
    ...
    // Per-request n-gram speculation override (tri-state): -1 = use the global speculative.ngram
    // default, 0 = force OFF, 1 = force ON. Lets requests opt in/out independently on the same
    // server. Resolved via Engine::spec_ngram_enabled_.
    // Per-request `"speculative": true/false`: -1 = unset, 0 = off, 1 = on. Named for the field,
    // not one drafter: false disables n-gram, MTP head, AND token recycling (#1639); true enables
    // whatever the model and operator config allow (cannot conjure an MTP head the checkpoint lacks).
    int spec_override = -1;
    // Per-request MTP chain depth from `"speculative": {"mtp_k": N}` (-1 = server's
    // speculative.mtp_k). Orthogonal to spec_override: object form addresses the trained head
    // only. Resolved against mtp_resolve_request (runtime/spec_request.h); engine clamps to it.
    int spec_mtp_k = -1;
    // Why this request did not get the speculation it asked for. Set once at request build,
    // travels back in usage.completion_tokens_details.imp_spec_declined. kNone means both
    // "got what it asked for" and "asked for nothing".
    SpecDecline spec_decline = SpecDecline::kNone;
    // Admission priority (vLLM semantics: LOWER value schedules earlier, default 0). Primary
    // sort key in Scheduler::schedule; aging and shortest-first order only WITHIN a priority
    // class. Strict dominance is deliberate: a caller that sets priorities owns cross-class
    // starvation, same as vLLM. From the "priority" body field on all three server dialects.
    int priority = 0;
    ...
    // OpenAI Predicted Outputs: client-supplied predicted completion tokens, never forwarded
    // through the model. They only extend the n-gram draft search corpus (input + prediction +
    // output); verify semantics are unchanged, output stays a faithful greedy decode.
    ...
    // Generation began inside an injected <think> prefix (the opener lives in the PROMPT, not
    // the output): seeds the think-budget recount loop and the CUDA-graph decode config so the
    // budget engages from token 0.
    bool started_in_think = false;
    // Sliding-window decoded-text buffer for multi-token </think> detection. Required for
    // tokenizers shipping <think>/</think> as added_tokens with special=False (Qwen3.6,
    // Qwen3-Coder NVFP4): the model emits </think> as multiple BPE tokens, so a single-token-id
    // compare can never trip. Accumulates the last ~32 decoded chars and scans for the literal.
    std::string think_text_tail;
    // Output-tokens index at which the think block was last exited, or -1 if never in one.
    // Enforces a minimum answer budget after </think> on numerically-noisy NVFP4 quants that
    // sometimes close an empty thinking block and would otherwise EOS to a 0-content completion.
    int think_exit_idx = -1;
    // Whether a real (non-stop) answer token has been emitted since the last </think>. The
    // post-think grace releases the moment this is true, so a complete short answer stops on
    ...
    // Context this request LOST to StreamingLLM eviction mid-generation (KV of the middle of
    ...
    // publishes each token behind __threadfence_system + a separate poll counter) (#754).
    bool stream = false;
    ...
    // Rerank scoring (/v1/rerank): prefill only, reads logits of these token ids at the LAST
    // position into score_out. Rides the same no-sampling path as embeddings, but DOES want the
    // prefix cache since reranking N documents replays one shared system+query prefix N times.
    ...
    std::string grammar;
    // Per-request constraint FSM (JsonConstrainer/SchemaConstrainer wrapper), owned by the
    // request so concurrent prefills/finishes of OTHER requests cannot clobber state and batched
    // decode can mask per row. Checked out of Engine::constraint_pool_ on first need, returned at finish.
    std::shared_ptr<class ConstraintManager> constraints;

    // Tool-call coordination: when true and (json_mode || !json_schema.empty()), the preamble
    ...
    // `{"name":...,"arguments":{...}}` + `tool_envelope_close`) via a TOOL_CALL schema FSM.
    // Each entry is (tool name, parameter-schema JSON); takes precedence over json_mode/json_schema,
    // falls back to the prompt hint when not enforceable (see build_tool_call_schema).
    ...
    // Strict OPTIONAL tool call (OpenAI `strict: true`, tool_choice=auto): the envelope is NOT
    // forced, the model may answer in text, but IF it opens the tool tag the body FSM enforces
    // the arguments. false = forced (mandatory).
    ...
    // Qwen-Coder XML dialect: body inside <tool_call> is
    // `<function=NAME><parameter=KEY>raw value</parameter>...</function>` (raw-text values).
    // Enforce with XML_TOOL_CALL grammar instead of JSON body FSM (masks newlines, mangles code).
    ...
    // Vision (multimodal), per-request binding. `image` carries CPU-preprocessed pixels; the
    // batch worker encodes it into `vision_emb` (device [n_vision_tokens, d_model] fp16) on
    // admission and clears `image`. step_prefill_one binds vision_emb at offset==0; shared_ptr
    // auto-frees on the last reference. Null for text-only requests.
    std::shared_ptr<ImageData> image;
    std::shared_ptr<Buffer> vision_emb;
    int32_t vision_token_id = -1;
    int n_vision_tokens = 0;

    // Qwen3-VL, per-request. `qwen_patches` holds CPU-preprocessed images in prompt order; the
    // batch worker encodes them into ONE concatenated `vision_emb` (+ one `deepstack_emb` per tap)
    // on admission and clears them. Per-request, not engine-global, since the server admits image
    ...
    int lora_id = 0;
    // Seed of the prefix-cache hash chain (image hash + active adapter), computed once by
    // Engine::add_request. Every lookup and publish site reads this field, so a prompt can only
    // reuse KV computed under the same picture and adapter.
    ...
    // What to add to a generated token's position to continue the M-RoPE sequence. An image
    // costs max(rows, cols) positions but occupies rows*cols tokens, so this is negative when
    // the prompt held one; `context_len()` alone would leave a gap the model never saw in training.
    int mrope_pos_delta = 0;
    // The same number, on the device, owned by THIS request: engine-global would be
    // cross-request corruption, since decode replays read the pointer at replay time and a
    // concurrent request's prefill would overwrite a value another sequence is mid-generation on.
