...
std::unique_ptr<VisionModel> load_vision_gguf(const std::string& path) {
    ...
    IMP_LOG_INFO("Vision GGUF: %lu tensors, %lu metadata", (unsigned long)tensor_count,
    ...
    IMP_LOG_INFO(
        "Vision config: image=%d, patch=%d, hidden=%d, heads=%d, "
    ...
    for (const auto& info : tensor_infos) {
        ...
        if (name == "v.patch_embd.weight") {
            model->patch_embd_w = t;
            assigned++;
        } else if (name == "v.patch_embd.bias") {
            model->patch_embd_b = t;
            assigned++;
        } else if (name == "v.position_embd.weight") {
            model->position_embd = t;
            assigned++;
        }
        // Post LayerNorm
        else if (name == "v.post_ln.weight") {
            model->post_norm_w = t;
            assigned++;
        } else if (name == "v.post_ln.bias") {
            model->post_norm_b = t;
            assigned++;
        }
        // Multimodal projector
        else if (name == "mm.0.weight") {
            model->mm_proj_w = t;
            assigned++;
        } else if (name == "mm.0.bias") {
            model->mm_proj_b = t;
            assigned++;
        ...
        // Layer weights: "v.blk.{i}.{field}"
            ...
            int layer_idx = 0;
            try {
                layer_idx = std::stoi(name.substr(6, dot2 - 6));
            } catch (...) {
                continue;
            }
            ...
            if (field == "ln1.weight")
                layer.ln1_w = t;
            else if (field == "ln1.bias")
                layer.ln1_b = t;
            else if (field == "ln2.weight")
                layer.ln2_w = t;
            else if (field == "ln2.bias")
                layer.ln2_b = t;
            else if (field == "attn_q.weight")
                layer.wq = t;
            else if (field == "attn_q.bias")
                layer.bq = t;
            else if (field == "attn_k.weight")
                layer.wk = t;
            else if (field == "attn_k.bias")
                layer.bk = t;
            else if (field == "attn_v.weight")
                layer.wv = t;
            else if (field == "attn_v.bias")
                layer.bv = t;
            else if (field == "attn_out.weight")
                layer.wo = t;
            else if (field == "attn_out.bias")
                layer.bo = t;
            else if (field == "ffn_up.weight")
                layer.ffn_up_w = t;
            else if (field == "ffn_up.bias")
                layer.ffn_up_b = t;
