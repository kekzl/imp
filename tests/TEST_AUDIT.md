...
| `test_engine_chat.sh`, `test_server_concurrent.sh`, `test_server_embed_chat_interleave.sh`, `test_server_0token_battery.py`, `test_server_robustness.py`, `exercise_all_endpoints.py`, `tests/api/elchtest.sh` | called by no target/CI (need a running server + GPU) | manual tools → documented as manual in each script header, do not delete. `exercise_all_endpoints.py` is also the `make coverage` driver (`scripts/coverage_server.sh`). |
