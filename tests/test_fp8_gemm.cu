...
// The SSM_IN failure: a coherent (positive-mean) activation against weight
// rows with a tiny absmax. The scalar-scaled FP8 GEMM into FP16 held
// out / row_scale = (K * 0.5 * 0.01) / (0.01 / 448) ~ 4.6e5 before the row
// scales were folded in, which is inf in FP16 (the uniform-logits PPL of
// 2026-08-31). The row-scaled GEMM applies them in the f32 epilogue.
TEST_F(FP8GemmTest, SsmPrefillFp8TinyRowsStayFinite) {
    const int M = 64, N = 256, K = 2048;
    ...
    for (int i = 0; i < M * K; i++)
        h_a[i] = __float2half(0.5f + 0.3f * frand());
    for (int r = 0; r < N; r++) {
        const float row_mag = (r % 2 == 0) ? 0.01f : 1.0f;
        for (int k = 0; k < K; k++)
            h_w[r * K + k] = __float2half(row_mag * (0.5f + 0.3f * frand()));
    }
    void *d_a, *d_w, *d_w8, *d_a8, *d_c, *d_cref;
    float *d_row_scales, *d_act_scale, *d_bm, *d_am;
    ASSERT_EQ(cudaMalloc(&d_a, M * K * sizeof(half)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_w, N * K * sizeof(half)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_w8, N * K), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_a8, M * K), cudaSuccess);
    ...
    const int max_grid = ((M * K + 3) / 4 + 255) / 256;
    ...
    Tensor A(d_a, QType::F16, 2, a_shape, true), W(d_w, QType::F16, 2, w_shape, true);
    Tensor C(d_c, QType::F16, 2, c_shape, true), Cref(d_cref, QType::F16, 2, c_shape, true);
    gemm_cublaslt(A, W, Cref, 1.0f, 0.0f, nullptr, nullptr, stream_);
    quantize_fp8_rows_async(d_w, d_w8, N, K, d_row_scales, stream_);
    Tensor A8(d_a8, QType::FP8_E4M3, 2, a_shape, true), W8(d_w8, QType::FP8_E4M3, 2, w_shape, true);
    ...
    const size_t ws_bytes = gemm_fp8_rowscaled_workspace_bytes(N);
    ASSERT_EQ(cudaMalloc(&d_ws, ws_bytes), cudaSuccess);
    // Mechanism: the same operands through a scalar-scaled FP8 GEMM with an
    // FP16 output (the 2026-08-31 form, row scales folded in afterwards)
    // overflow on the tiny rows - that is the bug this test pins.
    {
        void* d_c16 = nullptr;
        ASSERT_EQ(cudaMalloc(&d_c16, M * N * sizeof(half)), cudaSuccess);
        Tensor C16(d_c16, QType::F16, 2, c_shape, true);
        gemm_cublaslt(A8, W8, C16, 1.0f, 0.0f, d_act_scale, d_unit, stream_);
        ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
        std::vector<half> h_c16(M * N);
        cudaMemcpy(h_c16.data(), d_c16, M * N * sizeof(half), cudaMemcpyDeviceToHost);
        int inf16 = 0;
        for (int i = 0; i < M * N; i++)
            if (!std::isfinite(__half2float(h_c16[i])))
                inf16++;
        printf("[fp8 tiny rows] fp16-intermediate form: %d of %d outputs non-finite\n", inf16, M * N);
        EXPECT_GT(inf16, 0) << "the FP16-intermediate form no longer overflows; the test lost its defect";
        cudaFree(d_c16);
    }
    ASSERT_TRUE(gemm_fp8_rowscaled(A8, W8, C, d_act_scale, d_row_scales, d_unit, d_ws, ws_bytes, stream_));
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    cudaFree(d_unit);
    cudaFree(d_ws);
    ...
    int non_finite = 0;
    double se_tiny = 0.0, ref_sq_tiny = 0.0, se_big = 0.0, ref_sq_big = 0.0;
    for (int i = 0; i < M * N; i++) {
        const float got = __half2float(h_c[i]), ref = __half2float(h_cref[i]);
        if (!std::isfinite(got))
            non_finite++;
        const double d = (double)got - ref;
        if ((i % N) % 2 == 0) {
            se_tiny += d * d;
            ref_sq_tiny += (double)ref * ref;
        } else {
            se_big += d * d;
            ref_sq_big += (double)ref * ref;
        }
    }
    {
        std::vector<float> h_rs(N);
        cudaMemcpy(h_rs.data(), d_row_scales, N * sizeof(float), cudaMemcpyDeviceToHost);
        for (int n = 0; n < 6; n++)
            printf("[fp8 tiny rows] col %d: ref=%.4f got=%.4f row_scale=%.3e  ratio got/ref=%.4f\n", n,
                   __half2float(h_cref[n]), __half2float(h_c[n]), h_rs[n],
                   __half2float(h_c[n]) / __half2float(h_cref[n]));
    }
    printf("[fp8 tiny rows] non_finite=%d rms_rel tiny=%.3e big=%.3e\n", non_finite,
           std::sqrt(se_tiny / ref_sq_tiny), std::sqrt(se_big / ref_sq_big));
    EXPECT_EQ(non_finite, 0);
    EXPECT_LT(std::sqrt(se_tiny / ref_sq_tiny), 0.06);
    EXPECT_LT(std::sqrt(se_big / ref_sq_big), 0.06);
    cudaFree(d_a); cudaFree(d_w); cudaFree(d_w8); cudaFree(d_a8); cudaFree(d_c); cudaFree(d_cref);
    cudaFree(d_row_scales); cudaFree(d_act_scale); cudaFree(d_bm); cudaFree(d_am);
}
