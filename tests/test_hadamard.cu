...
static void hadamard_ref(const std::vector<float>& input, std::vector<float>& output, int M, int K,
                         int block_size) {
    output = input;
...
TEST_P(HadamardTest, MatchesCPUReference) {
    ...
    CUDA_CHECK(cudaMemcpy(d_input, h_fp16.data(), M * K * sizeof(half), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(h_out_fp16.data(), d_output, M * K * sizeof(half), cudaMemcpyDeviceToHost));
    ...
    EXPECT_LT(max_err, tol) << "block_size=" << block_size << " max_err=" << max_err;
...
TEST_P(HadamardTest, InPlaceWorks) {
    ...
    CUDA_CHECK(cudaMemcpy(d_buf1, h_fp16.data(), M * K * sizeof(half), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(d_inplace, h_fp16.data(), M * K * sizeof(half), cudaMemcpyHostToDevice));
    hadamard_transform_fp16(d_inplace, d_inplace, M, K, block_size);
    CUDA_CHECK(cudaDeviceSynchronize());

    // Compare
    std::vector<half> h_out1(M * K), h_out2(M * K);
    CUDA_CHECK(cudaMemcpy(h_out1.data(), d_buf2, M * K * sizeof(half), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_out2.data(), d_inplace, M * K * sizeof(half), cudaMemcpyDeviceToHost));

    for (int i = 0; i < M * K; i++) {
        EXPECT_EQ(__half2float(h_out1[i]), __half2float(h_out2[i])) << "Mismatch at index " << i;
    }
...
TEST_P(HadamardTest, InvolutionProperty) {
    ...
    CUDA_CHECK(cudaMemcpy(d_buf, h_fp16.data(), M * K * sizeof(half), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(h_out.data(), d_buf, M * K * sizeof(half), cudaMemcpyDeviceToHost));
    ...
    EXPECT_LT(max_err, 0.15f) << "block_size=" << block_size << " involution max_err=" << max_err;

    cudaFree(d_buf);
}

INSTANTIATE_TEST_SUITE_P(BlockSizes, HadamardTest, ::testing::Values(16, 32, 64, 128));
