...
// --- Cluster path coverage (M5 Slice 2) -----------------------------------
//
// The cluster kernel only fires when:
//   - n_q_per_kv ∈ {2,4,8}
...
TEST_F(FmhaSm120Test, ClusterPathNonAligned) {
    // Non-multiple-of-Bkv seq_kv (520 = 8×64 + 8) — exercises the tail
    // tile's out-of-range zero-pad logic in load_kv_tile.
    //
    // Tolerance bumped to 0.1 (vs the 2e-2 default) because the CPU
    // reference produces some near-zero ground-truth values where any
    // FP16-rounded GPU output yields max relative error > 1. The cluster
...
TEST_F(FmhaSm120Test, ClusterMatchesLegacyNonAligned) {
    // Diagnostic: explicit cluster-vs-legacy comparison for the NonAligned
    // shape that fails ClusterPathNonAligned. If the two paths agree to
    // FP16 tolerance, the CPU-ref delta is amplified by tiny |ref|
    // denominators rather than a real correctness regression.
    if (sm_ < 90) GTEST_SKIP() << "Requires sm_90+";

    const int B = 1, Sq = 100, Skv = 520, NH = 8, NKV = 2, HD = 128;
    ...
    for (size_t i = 0; i < q_elems; i++) {
        ...
        float rd = ad / std::max(std::abs(b), 1e-6f);
        max_abs_diff = std::max(max_abs_diff, ad);
        max_rel_diff = std::max(max_rel_diff, rd);
        if (rd > 0.05f) large_diff_count++;
    }
    fprintf(stderr, "ClusterMatchesLegacyNonAligned: max_abs=%g max_rel=%g large_count=%d/%zu\n",
            max_abs_diff, max_rel_diff, large_diff_count, q_elems);
    EXPECT_LT(max_abs_diff, 1e-2f) << "Cluster vs legacy max abs diff " << max_abs_diff;
...
TEST_F(FmhaSm120Test, ClusterMatchesLegacy) {
    // Stronger guarantee than the CPU-reference check: cluster vs legacy
    // path on identical inputs must agree within FP16 rounding (the two
    // kernels share the same WMMA fragment ordering, so the diff should be
    // bit-zero modulo softmax accumulation reordering across the cluster
    // sync barriers; tolerate 1e-3 relative).
    if (sm_ < 90) GTEST_SKIP() << "Requires sm_90+";
    ...
    EXPECT_LT(max_rel, 1e-2f) << "Cluster vs legacy max relative error " << max_rel;
