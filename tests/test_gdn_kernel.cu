...
void gdn_scan_cpu(const float* V, const float* K, const float* Q, float alpha, float beta_raw, float A_log,
                  float dt_bias,
...
int main() {
    ...
    for (int i = 0; i < inner; i++)
        h_V[i] = (rand() % 200 - 100) / 100.0f;
    for (int i = 0; i < BC_size; i++)
        h_K[i] = (rand() % 200 - 100) / 100.0f;
    for (int i = 0; i < BC_size; i++)
        h_Q[i] = (rand() % 200 - 100) / 100.0f;
    ...
    for (int h = 0; h < n_heads; h++) {
        int g = h % n_groups;
        gdn_scan_cpu(h_V + h * head_dim, h_K + g * state_size, h_Q + g * state_size, h_alpha[h], h_beta[h],
                     h_A_log[h], h_dt_bias[h], h_state_cpu + h * state_size * head_dim, y_cpu + h * head_dim,
                     head_dim, state_size);
    }
    ...
    printf("  First 4: CPU=[%.6f, %.6f, %.6f, %.6f] GPU=[%.6f, %.6f, %.6f, %.6f]\n", y_cpu[0], y_cpu[1],
           y_cpu[2], y_cpu[3], __half2float(h_y_gpu_h[0]), __half2float(h_y_gpu_h[1]),
           __half2float(h_y_gpu_h[2]), __half2float(h_y_gpu_h[3]));
    printf("  Max error: %.6f at index %d (CPU=%.6f GPU=%.6f)\n", max_err, max_err_idx, y_cpu[max_err_idx],
           __half2float(h_y_gpu_h[max_err_idx]));
    ...
    printf("\n=== Multi-token sequential test (5 tokens) ===\n");
    {
        ...
        for (int i = 0; i < n_tok * inner; i++)
            all_V[i] = (rand() % 200 - 100) / 100.0f;
        for (int i = 0; i < n_tok * BC_size; i++)
            all_K[i] = (rand() % 200 - 100) / 100.0f;
        for (int i = 0; i < n_tok * BC_size; i++)
            all_Q[i] = (rand() % 200 - 100) / 100.0f;
        for (int i = 0; i < n_tok * n_heads; i++)
            all_alpha[i] = (rand() % 200 - 100) / 100.0f;
        for (int i = 0; i < n_tok * n_heads; i++)
            all_beta[i] = (rand() % 200 - 100) / 100.0f;

        // CPU: process each token sequentially
        for (int t = 0; t < n_tok; t++) {
            for (int h2 = 0; h2 < n_heads; h2++) {
                int g2 = h2 % n_groups;
                gdn_scan_cpu(&all_V[t * inner + h2 * head_dim], &all_K[t * BC_size + g2 * state_size],
                             &all_Q[t * BC_size + g2 * state_size], all_alpha[t * n_heads + h2],
                             all_beta[t * n_heads + h2], h_A_log[h2 % n_heads], h_dt_bias[h2 % n_heads],
                             &state_cpu2[h2 * state_size * head_dim], y_cpu2 + h2 * head_dim, head_dim,
                             state_size);
            }
        }

        // GPU: process each token via gdn_scan_decode_f32
        cudaMemcpy(d_state, state_gpu2.data(), n_heads * state_size * head_dim * sizeof(float),
        ...
        for (int t = 0; t < n_tok; t++) {
            ...
            gdn_scan_decode_f32(d_V, d_K, d_Q, d_alpha, d_beta, d_A, d_dt, d_state, d_y, nullptr, n_heads,
                                head_dim, state_size, n_groups, 0);
            cudaDeviceSynchronize();
        }
