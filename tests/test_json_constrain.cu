...
TEST(JsonConstrainTest, ClassifyDigit) {
    ...
    EXPECT_TRUE(cat & CAT_NUMBER_START)
        << "Digit '0' should be classified as NUMBER_START";
    EXPECT_TRUE(cat & CAT_NUMBER_CONT)
        << "Digit '0' should be classified as NUMBER_CONT";
}
...
TEST(JsonConstrainTest, ClassifyBrace) {
    ...
    EXPECT_TRUE(cat & CAT_OPEN_BRACE)
        << "'{' should be classified as OPEN_BRACE";
    EXPECT_FALSE(cat & CAT_CLOSE_BRACE)
        << "'{' should NOT be CLOSE_BRACE";

    uint16_t cat2 = classify_token("}");
    EXPECT_TRUE(cat2 & CAT_CLOSE_BRACE)
        << "'}' should be classified as CLOSE_BRACE";
}
...
TEST(JsonConstrainTest, ClassifyStringContent) {
    ...
    EXPECT_TRUE(cat & CAT_STRING_CHAR)
        << "'hello' should be classified as STRING_CHAR";
    // Also a literal continuation (all lowercase)
    EXPECT_TRUE(cat & CAT_LITERAL_CONT)
        << "'hello' (all lowercase) should also be LITERAL_CONT";
}
