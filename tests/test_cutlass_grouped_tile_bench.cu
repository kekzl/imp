...
TEST_F(GroupedTileBench, Qwen36MoeShapes) {
    ...
    for (const auto& s : shapes) {
        ...
        arm("128x128 ping", v128ping);
        {
            ...
            // (per-expert SfAtom, tensor scale 1) - loose tolerance.
            bool ok = imp::gemm_nvfp4_smallm_v2_grouped(ne, ex.d_w_slab, ex.w_stride, ex.d_s_slab,
                                                        ex.s_stride, ex.d_w_ts, ex.xq_plain.packed_data,
            ...
            if (!ok || cudaGetLastError() != cudaSuccess) {
                ...
                cudaStreamSynchronize(stream_);
                float rel = max_abs_diff(ex.d_out, ref, out_n);
                // Split kernel error from quantisation difference: the dense v2
                // kernel per expert on the same plain operands must match the
                // grouped kernel exactly (same body, same tile order).
                ...
                for (int e = 0; e < ne; ++e) {
                    ...
                    xv.packed_data = static_cast<uint8_t*>(ex.xq_plain.packed_data) +
                                     (size_t)e * s.m_per * s.K / 2;
                    xv.micro_scales = static_cast<uint8_t*>(ex.xq_plain.micro_scales) +
                                      (size_t)e * s.m_per * s.K / 16;
                    ASSERT_TRUE(imp::gemm_nvfp4_smallm_v2_a4_tuned(ex.q[e], xv,
                                                                   y_dense + (size_t)e * s.m_per * s.N,
                ...
                float rel_dense_vs_cutlass = max_abs_diff(y_dense, ref, out_n);
                cudaFree(y_dense);
                printf(
                    "[grouped-tile] %-16s %-12s: %.1f us (prod 128x128x128 %.1f us, %+.1f%%)  max_rel vs "
                    "cutlass=%.2e"
                    "  vs dense v2=%.2e  (dense v2 vs cutlass %.2e)\n",
                    s.name, "v2 grouped", t, t_prod, 100.f * (t - t_prod) / t_prod, rel, rel_dense,
                    rel_dense_vs_cutlass);
                EXPECT_LT(rel_dense, 1e-3f)
                    << "v2 grouped differs from the dense v2 kernel on the same operands";
            }
        }
