...
### Changed
- `make test-e2e` (GDN container) runs `HybridBatchedDecodeTest.RowsOfOnePromptDecodeIdentically`: eight rows of one prompt under the batched GDN decode must decode row-identical (greedy, `runtime.deterministic`); the solo-vs-batched difference is printed only (2 of 8 rows on Qwen3.5-4B-mxfp4, rounding class). `tools/analysis/think_loop_probe.py` reproduces #2019 against a server.
