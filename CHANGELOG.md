...
- CUTLASS device-args MoE prefill writes the per-expert SFA base pointers in the offsets scan; `build_sfa_bases_kernel` is gone (2 launches fewer per MoE layer). gpt-oss-20b PPL 256.2186 both; tok/s not resolvable, eager pp512 spreads +-20 % on one image (#2117).
