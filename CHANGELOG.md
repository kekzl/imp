...
### Fixed
- The KV-pressure valve counted each live sequence's whole remaining `max_tokens` as need, so a 118676-token prompt with `max_tokens` 12000 lost 114768 tokens although its reply stopped at 206. It now looks 512 tokens ahead (`kKvValveHorizonTokens`): 0 evicted, needle found. `GraphEligibility.UnmetBlocksLookOnlyAHorizonAhead`.
