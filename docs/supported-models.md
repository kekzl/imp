...
| [Nemotron-3.5-Lightning-30B-A3B](https://huggingface.co/nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4) | NVFP4+FP8 | 21 GB | 137 | SafeTensors (Modelopt **MIXED_PRECISION**), same arch as Nano at 1M context. First checkpoint here with native FP8 *weights*: 46 Mamba in/out projections FP8, 5935 expert tensors NVFP4. The FP8 half gets an FP16 companion at load (sm_120 has no FP8 prefill GEMM) — 1698 MiB of cache, init at 24.4 of 32.6 GB. **Decode currently reads that FP16 copy, not the checkpoint's FP8 bytes** — twice the bytes per weight on the bandwidth-bound path, and the reason this lands below the older Nano's 148 despite being the "Lightning" release; see `docs/roadmap.md`. Its MTP head is not read (270 tensors, logged as unrecognised). |
