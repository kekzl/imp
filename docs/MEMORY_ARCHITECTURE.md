...
| 4b.2 — the remaining `exec/` workspaces | **in progress, cluster by cluster** (B52–B57): row-batched sampler mirror, sampling scratch, ten batched-MoE arrays, the FP8 reduction trio, the dp4a staging family + split-K, the MLA quartet + absorbed cache. 695 → **638** gate calls, acquisitions 309 → **284**; `executor_workspace_buffers.cu` 29 → **4** acquisitions, and two of B48's eight release-only files cleared. **What is left there is not a missed migration:** two `cudaHostAlloc` buffers have no tier to move to (T1/T2/T4 are device arenas — a pinned-host tier is a real gap, B57), and the `chunk_capture` pair is growable at 64 MiB a step, which a bump arena strands | `fix(exec): the row-batched sampler mirror / the sampling scratch / the batched-MoE array cluster / the FP8 reduction cluster / the dp4a staging cluster / the MLA cluster to the T2 arena` |
...
| 8 — `compute/` statics | **in progress, cluster by cluster**: the DRY penalty pair (B72), the cuBLAS/CUTLASS workspace family (B73 — plus the lazy CUTLASS growth path, deleted as unreachable) and **AUDIT B13's grow-on-demand statics (B74), which closes that bug class**: the IMMA activation/split-K scratches and the MXFP4 promotion scratches no longer free an address that is a live kernel parameter in an instantiated graph. Remaining in `compute/`, all per-kernel scratch rather than workspaces: `gemm_grouped_nvfp4_smallM.cu`, `sampling_topk_topp.cu`, `attention_cublas.cu`, `moe_routing.cu`, `mtp_forward.cu`, `rowwise_topm.cu`, the four constrainers | `fix(compute): DRY penalty buffers to the T2 arena` / `fix(compute): the cuBLAS and CUTLASS workspaces to the T2 arena` |
