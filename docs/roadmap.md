...
| 15 | multi-turn prefix reuse breaks on non-canonical BPE merges | OPEN, priced 2026-09-15. A generated reply re-tokenizes differently where the model emitted a non-canonical split: Qwen3.8-27B wrote ` pre` + `pref` (ids 829 29554), the tokenizer re-encodes ` prepref` as ` prep` + `ref` (20671 1062), so the next turn's hash chain breaks at that block (`lookup chain breaks at block 13 of 80`) and the whole reply re-prefills (`cached_tokens` 0; the token COUNT still matches, 1186 = 1186). Seen in 1 of 8 sessions in roughly 1 run of 4 on the pipeline path (long forced-close replies). Lever: remember the generated ids per finished reply and splice them in when a follow-up turn resends that reply verbatim, instead of re-tokenizing its text; the template's `<\|im_start\|>` markers are the natural splice points | #2028 |
