...
| Open 7, quantizer refuses 3-D stacked experts | 2026-09-16 | gpt-oss-20b BF16 -> NVFP4 in one `imp-quantize` run (2400 tensors, 12 820 MiB): PPL 179.23 vs 312.50 for the MXFP4 GGUF, verify-fast + paired A/B neutral. Per-model_type destack descriptor (`expert_destack.cpp`); the Gemma-4 layout is described but unrun (no BF16 source on the host). #2040 |
...
| Open 14, hybrid mixed prefill+decode step | 2026-09-15 | BUILT AND REFUTED: `runtime.prefill_mixed_decode_hybrid` (opt-in) -4.6% aggregate (868.6 / 869.7 / 874.1 vs 840.6 / 822.5 / 831.5 tok/s), short-stream ITL max 126-128 -> 181-184 ms during an ingest; the rider rows take the ragged GDN chunk path instead of the batched decode kernel. #2032 |
