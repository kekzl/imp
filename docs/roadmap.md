...
0. **Concurrency scaling on the GDN hybrid vs vLLM: 1.58x gap at 32 streams,
   ...
   | prefill kernel utilization measured (the open "%-peak" gap in this table) | MEASURED | roofline run `1d5b9230_20260831_180644` (ncu clocks locked, 1 restart): dense NVFP4 prefill GEMM (CUTLASS block-scaled) 79.8% of measured FP4 peak @pp4096 / 64.9% @pp512 - NOT the hole; the holes are `gemm_cublas` on the hybrid @pp512 (24.8% roofline at 21.5% share, est. +13.9% window - the FP16 GDN projections) and `attn_fa2` @pp4096 dense (22.8% at 21.9% share, est. +12%). The fa2 prefill kernel runs at 255 registers / 16.7% occupancy (kernel-resource pin + ncu), and that occupancy is CLOSED, not tunable: 255 regs x 256 threads is the full 64K register file, and ncu on the big pp4096 launches (337 us) shows DRAM at 7.6%, compute 34%, 72% no-eligible cycles - latency-bound inside 8 warps/SM, not bandwidth. The cheap alternative (forcing the existing Bq=64/TWOSLOT band for 2 CTAs/SM at the same 8 warps) measures -2.0%/-1.3% pp4096, 2/2 pairs - CTA overlap loses to the extra softmax rescales. The remaining lever is deeper in-CTA pipelining (3-stage cp.async at Bkv=32, or QK/PV stage overlap). Full table: `tools/roofline/history/runs/1d5b9230_20260831_180644.json` | 2026-08-31 |
