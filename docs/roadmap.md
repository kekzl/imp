...
| 14 | hybrid mixed prefill+decode step | PRICED 2026-09-15, not built: the mixed step (row 2) refuses recurrent models. Lifting the refusal for GDN (a rider is a one-token continuation chunk, the chunked-prefill route) ran one mixed step (39 rows, 31 riders) and then threw `attention_prefill_dispatch: no prefill kernel accepts head_dim=256` on a 2048-row chunk with 4 riders, then an illegal memory access. Needs an hd256 prefill path that accepts one-token rows at long context. The shape it would serve: Qwen3.8-27B-NVFP4, 32 x 1k-token prompts x 300 tokens, 3 waves, 865.7 / 875.7 / 868.4 tok/s against 1900 with short prompts, so prefill dominates the hybrid burst | ledger 2026-09-15 |
