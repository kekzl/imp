...
| 4 | Constrained decoding is JSON-only | open, but **smaller than it looked** — the regex engine (`RegexNfa`) already exists; only the constrainer + wiring are missing |
...
4. **Constrained decoding is JSON-only — but the hard part already exists.** The FSMs cover `json_schema`, `json_object` and the XML tool dialect; there is no regex/GBNF surface, so an agent that needs a diff header, SQL, an ID format or a small DSL falls back to prompting. **Scoping correction (2026-07-26): a byte-level regex engine is already in the tree** — `RegexNfa` in `src/compute/json_schema.h`, used for JSON-Schema `pattern`. Verified directly against a from-scratch DFA implementation: identical results on literals, classes, negation, shorthands, alternation, grouping and `{n,m}` quantifiers, so it is sufficient as-is and a second engine was written and discarded rather than merged. What is actually missing is the *constrainer* around it: a `RegexConstrainer` beside `JsonConstrainer`/`SchemaConstrainer` with the same `apply_mask` contract (per-step token mask from the active NFA state set, cached per state set since a 151k-token walk per step is too slow otherwise), plus `ConstraintManager` wiring and a `response_format: {"type":"regex"}` entry point. That is a fraction of the original estimate. Separately, `/v1/rerank` is still absent and is NOT part of this item — it needs cross-encoder architecture support, which is model work, not a grammar surface.
