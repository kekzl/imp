...
| 2 | paced serving prefill, dense | HALF CLOSED (2026-09-08). Anatomy from a debug-log step timeline (Qwen3-14B-NVFP4, 32 x 982-token prompts, wave 2): each capped step prefills 1024 rows in 56-59 ms against a 38 ms standalone 1024-row forward (26.6k tok/s; 2048 rows 27.9k, 4096 rows 26.4k), the rest is the decode step and host turnaround the step carries; 29 such steps = 1.68 s ingest. SHIPPED `runtime.prefill_cap_fairness`: the cap scales by W x waiting / decoding; at W=1 22 steps (9 at 2048 rows, 86-117 ms each) = 1.52 s, A/B 3/3 +3.0..+4.2% aggregate, TTFT p50 -18..-23%, ITL max 106 -> 139 ms; W=4 (default) keeps the wave at 2048 until a quarter still waits: a further +1.9..+2.5%, TTFT p90 -9%, ITL max unchanged, gaps over 100 ms 9 -> 22 of 9568. Remaining: the decode step inside every prefill step (~20 ms of a 57 ms step). vLLM's ~1.1 s comes from decodes riding the prefill forward (one mixed batch, no separate decode step): a scheduler-and-attention project, not a cap | ledger 2026-09-08, #1950, #1951 |
