...
| 12 | hybrid pp512 `gemm_cublas` hole | HALF BUILT 2026-09-15, opt-in: the alpha/beta tails run as one split-K FP16 tensor-core launch plus one fixed-order reduce with separate outputs (`gdn.alpha_beta_prefill`, no consumer stride needed). Qwen3.8-27B pp512: 12.3 + 4.2 us per GDN layer against 2 x 11.1-12.6 us cuBLAS (nsys, 3 alternating pairs), ~0.6% of the forward; Qwen3.6-35B: 7.9 vs 7.4 us per layer, neutral (K=2048, N=32 is launch-bound either way). Kept off by default because the PPL judge moves: the shipped cuBLAS path accumulates in FP16 (`gemm.cublas_fp16_acc` auto) and reads 4.6242, the FP32-accumulate kernel 4.6404, cuBLAS forced to FP32 4.6426; both kernels sit at rms 7.4e-4 off the fp64 reference, so the delta is the accumulate precision the default happens to have, not an error. The other half (the 35-48%-of-peak tiles on `ssm_in`/`gate`/`ssm_out` of the BF16-recipe hybrid) still needs the packed 4-in-1 GEMM with a row stride on every consumer or a deinterleave pass | ledger 2026-09-02, `tools/analysis/prefill_kernel_ab.sh`, `GemmF16NarrowPrefill.BenchVsCublas` |
