...
0. ~~**Concurrency scaling on the GDN hybrid vs vLLM: 1.58x gap at 32 streams,
 ...
 | #1880 |
   ...
   at 32 with 1082-token prompts; Qwen3-14B-NVFP4 +7.9% at 8, and with the four-token FP8 decode kernel
   +3.4% at 32 but -25.5% at 32 with 982-token prompts, 3/3 each,
   `BENCHMARKS.md` runs 5-9); the "~1.08x pinned" of 2026-08-26 was a
   cross-client figure. Next engine-side post: the paced serving prefill on
   dense models (31.4k prompt tokens in ~2 s under decode against 26k tok/s
   standalone; vLLM absorbs the same wave's prefill in ~1.1 s). auto=28 vs pinned=32
