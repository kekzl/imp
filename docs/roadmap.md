...
- **BitDecoding** ([arxiv:2503.18773](https://arxiv.org/abs/2503.18773), HPCA 2026) — **HIGHEST-ROI item** in this section. Tensor-Core decode on dequantized NVFP4 KV: 8.6× over FP16 FlashDecoding-v2 on Blackwell (RTX 5090 in benchmarks), 3× E2E latency on Llama-3.1-8B 128K. imp's current NVFP4 KV (Lever 2) gets the VRAM win but uses CUDA cores → parity-only on decode tok/s; BitDecoding is the missing perf piece. Empirically confirmed 2026-05-09: SASS audit of `paged_attention_decode_nvfp4_kernel<128>` shows **346 scalar FP ops, 0 HMMA** (`tools/analysis/sass_nvfp4_paged_decode.sh`). The 8× upside is mechanical, not a paper-only claim. Stateless, paged-compatible, no retraining, sm_120 explicit. Adoption effort: ~2–4 wk hand-CUDA port (residual + packing + combined kernels; ref impl at [OpenBitSys/BitDecoding](https://github.com/OpenBitSys/BitDecoding)). Use the audit script after each port step to verify HMMA count moves up. Phase-0 microbench shipped 2026-05-09 (`tools/analysis/bench_nvfp4_qk_tc_vs_scalar.sh`): scalar-FFMA vs WMMA-HMMA Q.K dot on synthetic NVFP4 input; numerical equivalence within 1e-4 rel; isolated TC dispatch alone runs at **0.82× scalar** at HEAD_DIM=128 seqlen_kv=4096 (i.e. slower) — anticipated, the BitDecoding 8.6× requires four combined levers (TC dispatch + software pipeline + multi-warp N-dim + residual cache), not just lever 1. Phase 1 will measure in production attention shape where the picture differs. Plan: `docs/superpowers/plans/2026-05-09-bitdecoding-port.md`.
