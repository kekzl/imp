...
| 2 | paced serving prefill, dense | HALF CLOSED (2026-09-08). Anatomy from a debug-log step timeline (Qwen3-14B-NVFP4, 32 x 982-token prompts, wave 2): each capped step prefills 1024 rows in 56-59 ms against a 38 ms standalone 1024-row forward (26.6k tok/s; 2048 rows 27.9k, 4096 rows 26.4k), the rest is the decode step and host turnaround the step carries; 29 such steps = 1.68 s ingest. SHIPPED `runtime.prefill_cap_fairness`: the cap scales by waiting / decoding, 22 steps (9 at 2048 rows, 86-117 ms each) = 1.52 s; A/B 3/3 +3.0..+4.2% aggregate, TTFT p50 -18..-23%, ITL max 106 -> 139 ms. Remaining: the decode step inside every prefill step (~20 ms of a 57 ms step) and the second half of the wave still at 1024 once the waiters no longer outnumber the decoders. vLLM's ~1.1 s comes from decodes riding the prefill forward (one mixed batch, no separate decode step): a scheduler-and-attention project, not a cap | ledger 2026-09-08 |
