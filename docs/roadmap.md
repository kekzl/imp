...
0. **Concurrency scaling on the GDN hybrid vs vLLM: 1.58x gap at 32 streams,
   ...
   | FA2 in-CTA pipelining, measured from the 2-CTA instance's ncu profile | SHIPPED (log2 softmax + interior-tile fast path); QK interleave REFUTED | ncu on `fa2_kernel_2cta` @pp4096 Qwen3-14B-NVFP4 (per launch, 326 us): issue_active 41% (1-CTA: 28%), tensor pipe 54% (45%), warps eligible 0.67/SMSP; stalls per issue `wait` 2.12, `math_pipe_throttle` 1.87, `short_scoreboard` 1.22, `long_scoreboard` 0.34, i.e. fixed-latency scalar chains, not cp.async. Instruction mix per launch: ALU 60.9M, FMA 22.4M, MMA 11.1M, LSU 11.0M, XU 3.0M = 5.5 ALU per MMA, the softmax/mask loop. (a) QK^T MMA interleave (2/4 n-tile pairs in flight, `attention.fa2_qk_width`, removed): width 2 kernel sum 250.8/253.3 -> 259.1/256.9 ms (+1.4/+3.3%), width 4 257.1/252.0 -> 252.2/250.3 (-1.9/-0.7%), noise at the 128-register cap. (b) scale inside the exp FMA (`__expf(fmaf(s, scale, -m*scale))`) instead of a `v *= scale` pass, plus a CTA-uniform `tile_masked` predicate so interior tiles skip 4 compares + 2 selects per element: two-binary A/B pp4096 FA2 kernel sum 244.29/243.55 -> 225.56/226.08 ms (-7.7/-7.2%), pp 24396/24280 -> 24718/24770 tok/s; deterministic PPL `ppl_corpus_45k` Qwen3-14B 10.0277 -> 10.0229; Qwen3.8-27B-NVFP4 (hd=256 instance) pp4096 137.57/137.03 -> 118.10/117.70 ms (-14.2/-14.1%), pp 6734/6730 -> 6762/6759, deterministic PPL 4.6283 both arms. Two faster-looking forms refuted on quality (deterministic runs): scale*log2e folded into the Q fragments (f16) read -8.2/-7.8/-8.1% kernel but PPL 10.0277 -> 10.0151 on 14B and +0.37% on Qwen3.8 (small Q elements lose f16 precision), and the exp2 form `exp2f(fmaf(s, scale*log2e, -m*scale*log2e))` on raw scores read the same kernel time at PPL 10.0623 (+0.35%) with the mask fast path bisected out; the `__expf` form on the same FMA reads 10.0229. Trap: PPL WITHOUT `runtime.deterministic=true` moved 0.35% between two runs of the base binary (4.6283 vs 4.6447 on Qwen3.8), so every quality verdict here is a deterministic pair | 2026-09-01 |
   | FP8 prefill for the GDN projections (`gemm.fp8_ssm_prefill`) | REFUTED e2e | cuBLASLt FP8xFP8 measures 2.0-3.6x at the projection shapes (sm_120/CUDA 13.3), and the SSM_OUT arm holds quality (PPL -0.03% over 13.8k sliced tokens) - but 6/6 e2e pairs are negative (pp512 -7.4/-2.3/-0.7%, pp4096 -1.9/-0.2/-0.2%, tg flat): ssm_out is 16 of 48 FP16 MB per GDN layer and the per-chunk act-quant+rescale overhead eats the ~1% ceiling. The SSM_IN arm (32 MB/layer, the one that would pay) produced uniform logits (PPL 4.09 -> 248320); the act-outlier hypothesis is refuted by unit test, root cause unisolated. Closed unmerged, branch `perf/fp8-ssm-prefill` is the record; full tables [`plans/2026-08-31-fp8-ssm-prefill.md`](plans/2026-08-31-fp8-ssm-prefill.md) | 2026-08-31 |
