...
0. ~~**Concurrency scaling on the GDN hybrid vs vLLM: 1.58x gap at 32 streams,
   ...
   | F16 split-K CTA target 4 per SM for the four-head multitok instance | SHIPPED default-on | Same-session microbench (imp:skmt binary vs dev build): 32/8 HD=128 batch 1 16k 62.3 -> 46.7 us, 32k 98.6 -> 92.4, 64k 188.6 -> 172.3; 4 x 8k 117.8 -> 90.4. A blanket 4 per SM regressed the one-head (24/8: 96.9 -> 118.2) and two-head (16/8 HD=256: 169.1 -> 186.4) instances, so the doubling is tied to `mt_hpc == 4`; those read 98.4 / 169.0 under the rule. E2E_T4 | #PRNUM |
 | #1882 |
 | #1880 |
