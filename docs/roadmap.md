...
| NVFP4 prefill copies of the F16 GDN in_proj / gate / out_proj (`gemm.nvfp4_gdn_proj_prefill`) | `in` DEFAULT since 2026-09-17 (Qwen3.6-35B `in` alone pp4096 +7.7% 3/3; Qwen3.8-27B BF16-in_proj export +12.3% 3/3, PPL +0.56%; Open 12 row), gate / out / all opt-in on the PPL price | Qwen3.6-35B-A3B-NVFP4, `imp-cli --bench`, 3 alternating pairs: `in,gate` pp4096 146.66 / 147.18 / 146.18 -> 127.06 / 127.60 / 128.42 ms (+15.4%, 3/3); `all` 146.76 / 147.87 / 147.42 -> 121.63 / 122.87 / 121.37 ms (+20.9%, 3/3); pp512 under nsys (`in,gate`, 3 pairs): cuBLAS class per run 92.8 / 104.4 / 81.9 -> 42.9 / 44.1 / 46.0 ms, CUTLASS 80.4 / 80.1 / 81.6 -> 86.8 / 86.5 / 85.5 ms. PPL judge (deterministic, `ppl_corpus_45k`): 6.8543 -> in 6.9007, gate 6.8956, in+gate 6.9781, out 6.9955, all 7.1458 (+0.60..+4.25%). Copies 135 MiB per projection set of 30 layers, unbudgeted like the FP8 sidecar. Trap: `gemm_via_handle_` returns for tier-Undefined handles (ssm_in, gdn_gate: no cache entry) before the tier split, the route sits ahead of it | 2026-09-16 |
