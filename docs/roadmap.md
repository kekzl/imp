...
| 5 | No speculation tree / trained draft head | open — **re-measured 2026-07-27: still net-negative** (−7% on reasoning), and no longer the largest batch=1 lever (see #1100) |
...
| 10 | Agent-harness batteries are imp-internal | **largely closed** — real aider AND Claude Code binaries drive imp in `make test-agents-external`; a Responses-API client (Codex / Agents SDK) is the remaining leg |
...
5. **Speculation has no tree and no trained draft head.** No EAGLE / Medusa / multi-candidate path exists in the tree. Prompt-lookup n-gram only drafts spans that already appear in the context, so it contributes nothing on free-form reasoning output, and the verify-in-loop experiment was removed after a nine-class sweep found no prompt class where it won (see `CHANGELOG.md`, Unreleased).
    ...
    **And it is no longer the largest batch=1 lever.** The same measurement session found that decode throughput is set by the CONFIGURED context capacity rather than the live sequence length: the same 280-token request runs at 160 tok/s with `runtime.max_seq_len=1024` and 99 tok/s at the server's default 40960, with the captured decode body growing 44 → 137 kernels (issue #1100, repro `tools/analysis/ctx_capacity_decode_sweep.sh`). That is +61% available on the served path, against the tree's projected +20–50% on a subset of prompts. It also explains why this was invisible: `imp-cli --bench` sizes the engine to the bench workload, so the pinned baseline (165.10) and the served number (99.3) are measurements of different regimes. Fix #1100 before revisiting the tree.
