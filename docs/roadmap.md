...
| 8 | no audio | HALF CLOSED: the drop is now stated, the modality is still absent. `audio_config` as an OBJECT (Gemma-4-12B, `gemma4_unified_audio`; Gemma-4-26B writes `null` and must not warn) warns at config load, and `WeightMap::skip_stats()` splits the old aggregate into vision / audio / MTP / unrecognised: Gemma-4-12B-NVFP4 read "skipped 11" for 1 audio tensor (4.69 MiB) plus 10 vision-embedder tensors (95.22 MiB). Remaining: an encoder, an input type and a tokenizer route for `audio_token_id` | `tests/test_weight_map_modality_skips.cpp` |
