...
| 12 | hybrid pp512 `gemm_cublas` hole | CLOSED 2026-09-16, both halves measured. The hole is the kernel class, not the tiling: cuBLAS FP16 (FP16 accumulate, timed algo probe) reads 290-345 TFLOPS on every GDN projection shape, merged N or not (ledger "packed / merged-N" row), so the consumer-stride fusion is dead. The 4-bit tier is BUILT, opt-in: `gemm.nvfp4_gdn_proj_prefill` (`false`, `all`, or a list of in|gate|out) quantizes the F16 GDN projections of a native-NVFP4 hybrid to NVFP4 prefill copies at load (Qwen3.6-35B: 270 MiB for in, 135 gate, 135 out) and routes M>32 rows to the CUTLASS W4A4 GEMM; M<=32, M=1 and the spec verify chunk keep FP16 / the FP8 sidecar. Qwen3.6-35B-A3B-NVFP4, `imp-cli --bench`, 3 alternating pairs: `in,gate` pp4096 146.66 / 147.18 / 146.18 -> 127.06 / 127.60 / 128.42 ms (+15.4%, 3/3), `all` 146.76 / 147.87 / 147.42 -> 121.63 / 122.87 / 121.37 ms (+20.9%, 3/3); pp512 e2e is bimodal per process on this model (30-32 vs 38-40 ms in both arms, 20 reps do not settle it), the nsys class sums are the judge there: cuBLAS class per run (7 forwards) 92.8 / 104.4 / 81.9 -> 42.9 / 44.1 / 46.0 ms for CUTLASS 80.4 / 80.1 / 81.6 -> 86.8 / 86.5 / 85.5 ms (`in,gate`, 3 pairs). The price is quality, and it is the activations (roadmap 13: the e2m1 element grid is the floor): deterministic PPL on `ppl_corpus_45k` 6.8543 -> in 6.9007 (+0.68%), gate 6.8956 (+0.60%), in+gate 6.9781 (+1.81%), out 6.9955 (+1.98%), all 7.1458 (+4.25%). Default `in` since 2026-09-17: the second hybrid judged it, an `imp-quantize --keep-gdn-proj in` export of Qwen3.8-27B (BF16 in_proj, 4845 MiB kept; `all` keeps 10.6 GiB and no longer fits 32 GB), deterministic PPL 4.6143 -> 4.6400 (+0.56%), pp4096 373.51 / 373.66 / 373.40 -> 332.65 / 332.87 / 332.89 ms (+12.3%, 3/3 pairs, `gemm.fp8_ssm_proj=false` both arms for VRAM); Qwen3.6-35B `in` alone 147.04 / 146.03 / 145.86 -> 135.77 / 135.45 / 136.44 ms (+7.7%, 3/3). gate / out / all stay opt-in on their PPL. The W8A8 twin `gemm.mxfp8_gdn_proj_prefill` (E4M3 + UE8M0 per 32 on the same CUTLASS block-scaled path, same day) takes the prefill without the PPL: Qwen3.8-27B, one `--keep-gdn-proj` export per role, deterministic 45k, `gemm.nvfp4_lm_head=off` + sidecar off in every arm: `in` 4.5683 -> 4.5718 (+0.08%; NVFP4 4.5827, +0.32%), `gate` 4.5713 -> 4.5481 (-0.51%; NVFP4 +0.01%), `out` 4.5715 -> 4.5631 (-0.18%; NVFP4 +0.47%); pp4096 `in` 375.74 / 376.06 / 376.13 -> 353.70 / 353.52 / 353.49 ms (+6.0%, 3/3). Qwen3.6-35B pp4096 (3 pairs, sidecar off): MXFP8 `in` 137.88 / 137.29 / 137.57 vs NVFP4 `in` 135.13 / 133.63 / 133.62 ms (2.7% behind NVFP4, +6% over FP16), MXFP8 `all` 146.48 / 145.62 / 145.73 -> 130.00 / 131.03 / 130.62 ms (+11.6%; NVFP4 `all` +20.9% at +4.25% PPL). Opt-in on VRAM, not on quality: the copies are twice the NVFP4 bytes and unbudgeted, and at stock flags MXFP8 `in` (495 MiB) leaves the 35B's KV pool 50 blocks where NVFP4 `in` (270 MiB) leaves 225 (pp4096 phantom 0.10 ms). Default flip needs the copies budgeted or the F16 source dropped (M<=32 rows onto the copy). The 3.6k-token slice inverted the `in` verdict (MXFP8 +0.53% vs NVFP4 +0.31%): judge on the 45k corpus only. The alpha/beta half stays opt-in (`gdn.alpha_beta_prefill`, ~0.6%). What would take the +15% without the PPL: W8A8 FP8 with the activation quant fused into the norm producer (the per-chunk act-quant is what refuted `perf/fp8-ssm-prefill-v2`); W4A16 cannot beat cuBLAS FP16 at large M because the MMA is the FP16 pipe either way | ledger 2026-09-02 + 2026-09-16, `tools/analysis/prefill_kernel_ab.sh`, `GemmF16NarrowPrefill.BenchVsCublas` |

## Closed

| item | closed | verdict |
|---|---|---|
| Open 1, launch-coupled idle @32 | 2026-09-16 | CLOSED as measured: steady-window idle 3.6-4.0% (the "~8% real" was the benchmark's wave ramp). Shipped `runtime.penalty_append_early` (+0.5% median, #2022) and `runtime.decode_pipeline_hybrid` (+1.4% median, 4 of 6 pairs); one pinned H2D upload of the per-step tables refuted 3/3. Ledger "idle" row; full text in [plans/2026-09-16-open-rows-closed-detail.md](plans/2026-09-16-open-rows-closed-detail.md) |
| Open 2, paced serving prefill, dense | 2026-09-08 | `runtime.prefill_cap_fairness` + `runtime.prefill_mixed_decode`: 32 x 982-token ingest 1.68 -> 1.175 s (vLLM ~1.1), aggregate +6.6..+7.8%, TTFT p50 -20..-23%, ITL max 141 -> 80 ms. Left: the prefill forward itself at 80% of the FP4 peak. #1950, #1951, #1952 |
| Open 4, speculation adapts per request | 2026-09-15 | `mtp_k=auto` single-stream 95.8 -> 141.6 tok/s (+48%, #1801). The drafter choice has no per-request target: MTP +43% where text repeats and free elsewhere (140.4 / 97.6 / 101.5 vs 97.7 / 97.8 / 97.9 tok/s), n-gram never leaves the no-drafter line (acceptance 11.6%). #1809, #1811 |
| Open 7, quantizer refuses 3-D stacked experts | 2026-09-16 | gpt-oss-20b BF16 -> NVFP4 in one `imp-quantize` run (2400 tensors, 12 820 MiB): PPL 179.23 vs 312.50 for the MXFP4 GGUF, verify-fast + paired A/B neutral. Per-model_type destack descriptor (`expert_destack.cpp`). Gemma-4 arm run 2026-09-16 on `google/gemma-4-26B-A4B-it` BF16 (51.6 GB): 60 stacks split, 11726 tensors quantized, 747 copied, 15 693 MiB, worst max-rel 0.1711, 4 min 46 s; loads, greedy coherent, 198 tok/s decode; the dry run exposed the router (`router.proj.weight`) as QUANT, now excluded. PPL on the turn-framed prose corpus (Gemma-4 scores only inside its own turn framing, #2044): imp-quantize 26.99 vs reference NVFP4 export 26.06 vs UD-Q4_K_M 18.16. #2040 |
| Open 13, batched decode is W4A4, single-stream W4A16 | 2026-09-15 | PRICED AND CLOSED: A16 everywhere costs 38% at 32 streams (1899.7 / 1947.2 / 1903.4 vs 1186.7 / 1172.1 / 1172.8 tok/s), no cheap subset (A16 on the GDN projections alone leaves 0.45-1.05 nats, the FP16 LM head alone -21% for no gain); the e2m1 element grid is the floor (an exact fp32 block scale 0.0285 -> 0.0280 relL2). #2026 |
| Open 14, hybrid mixed prefill+decode step | 2026-09-15 | BUILT AND REFUTED: `runtime.prefill_mixed_decode_hybrid` (opt-in) -4.6% aggregate (868.6 / 869.7 / 874.1 vs 840.6 / 822.5 / 831.5 tok/s), short-stream ITL max 126-128 -> 181-184 ms during an ingest; the rider rows take the ragged GDN chunk path instead of the batched decode kernel. Re-open trigger: a head-dim-256 prefill attention kernel that lets rider rows run the batched decode path. #2032 |
| Open 15, multi-turn prefix reuse breaks on non-canonical BPE merges | 2026-09-15 | `server.transcript_token_reuse` (default on): a finished reply's ids splice into the resend, 48 of 48 turn-2 renders spliced, 40 of 40 sessions cached > 0; two publication races closed on the way (`Request::release_pending`). #2028, #2038 |
...
| fused QK-norm + RoPE for batched decode rows (`qknorm_rope_fused`, grid heads x tokens, gate n == 1 -> n <= 64 with full-head norm weights) | SHIPPED, small | The batched path ran q-norm, k-norm and `rope_forward` as three launches per attention layer (6.5 us from the q\|k\|v GEMM end to the attention start in the #1954 trace). Two images (main after #1955 vs branch), 3 alternating trials x 3 waves, IGNORE_EOS: Qwen3-14B-NVFP4 32 x 982 tokens 2639.8/2639.1/2639.5 -> 2661.5/2657.3/2631.5 (+0.8/+0.7/-0.3%), ITL p50 8.2 both; Qwen3.8-27B-NVFP4-vllm 32 x 40 tokens 1949.8/1950.3/1946.1 -> 1954.6/1954.3/1949.6 (+0.2 x3). 80 launches per step fewer on the 14B; the batched rows now take the FP32-intermediate norm + rope the single-stream path always had (`BatchInvarianceTest` W4A4 M=1/M=32 top-1 flips 3 -> 2 of 64, but that test's M=1/M=1 control arm itself flipped 2-3 of 64 on main at the time: the #1954 pipeline race below, not the deterministic mode; the M=32 deltas measured on that main are not attributable). `DegenerationTest.*` 5/5 | #1957 |
| small-M pipeline race (`gemm_nvfp4_smallm_v2`, every shape that runs the shared CTA body) | FIXED 2026-09-08 | Bisected over the per-PR images: `BatchInvarianceTest` control arm (M=1 vs M=1, `runtime.deterministic`) bit-exact through #1953, 1-3 of 64 greedy flips with max \|dlogp\| 0.3-0.7 from #1954 on; `runtime.no_pdl=true` changes nothing, reverting the prefetch split alone restores 0.000. The M=1 arm's decode runs the GEMV; its 24-token prefill runs q\|k\|v multi (112 CTAs), gate\|up multi (544 CTAs, two waves), o and down single at M=24, and the different KV bits per run reach all 64 steps. Isolated with the in-situ shapes, two input sets alternating: only the 544-CTA launch differs, 126 of 200 launches synced, 60 of 200 back to back; every single-wave shape 0. Mechanism: the helper warp's track-only `cp.async.mbarrier.arrive` increments the pending count only in its own program order, so when the 32 producer arrivals land first (a second-wave CTA behind a full LSU queue) the phase completes without the stage's weights. Fix: the full barrier expects 64 arrivals, the helper warp arrives `.noinc` on the stages it prefetched and the producer adds a plain arrive on stage 0 and every refill. Control arm 0.000 / 0.000 (re-init) on the fixed kernel, NLL 3.435474 both runs; isolated survey fix vs main (2 alternating pairs) gate\|up pair 1420.4/1434.6 vs 1397.4/1416.8 GB/s, down 1407.2/1386.2 vs 1412.1/1410.5, noise; two-image serving A/B (`prefill_cap_conc_ab.sh`, Qwen3-14B-NVFP4, 32 x 982 tokens, IGNORE_EOS, 3 trials x 3 waves, wave medians) 2582.7/2598.3/2563.8 -> 2620.9/2623.8/2552.5 tok/s (+1.5/+1.0/-0.4%), ITL p50 8.3 ms both arms | #1958, `NvFP4SmallMV2Test.RepeatedLaunchesBitwiseStable` |
...
| mean+std page score instead of the Quest min/max corner bound (arXiv 2605.27740) | SHIPPED default-on | `max(q*min, q*max)` is `q*(min+max)/2 + |q|*(max-min)/2`, so the bound's width per dimension is whichever single token is most extreme there; storing the page's own mean and standard deviation keeps the layout, the loads and the arithmetic count and changes only the metadata pass (Welford plus Chan's merge, `slot` is the count already covered). Qwen3.8-27B-NVFP4-vllm, one image, `niah_check.py` 5 depths x 81 908 / 126 908 prompt tokens, `sparse decode attention ACTIVE` asserted per arm: 4096 budget 2/10 -> 10/10, 8192 budget 7/10 -> 10/10. Wall on a 77k prompt at 128 emitted tokens, arms alternating over two rounds: 11.54/11.70/11.84/11.67 -> 11.53/11.54/11.56/11.69 s. The first attempt measured nothing (both arms 10/10) because `kv_cache.growable` defaults true and the metadata pool refuses a growable pool: the startup line proves the scratch, only `ACTIVE` proves the selection | [plan](plans/2026-08-28-sparse-decode-attention.md) |
...
| packed / merged-N cuBLAS GEMM for the FP16 GDN in+gate at prefill (the other half of Open 12) | REFUTED by microbench | cuBLAS FP16 (FP16 accumulate, the timed algo probe) reads 290-345 TFLOPS at every shape, 35-41% of the 838 TFLOPS peak, merged or not: Qwen3.6-35B K=2048 two calls vs one merged N=12288, M=512 87.3-99.2 vs 79.4-80.2 us, M=1024 155.6-160.5 vs 152.4-160.2, M=2048 307.9-314.9 vs 338.6-338.9, M=4096 705.9-712.2 vs 662.7-667.7; Qwen3.8-27B K=5120 M=512 276.1-285.1 vs 282.6-288.9, M=2048 1060.4-1071.2 vs 1252.8-1257.7 (weight ring > L2, 2 timings each). Not wave quantisation but the kernel class, so no consumer stride is worth building; the lever is the 4-bit tier (next row) | 2026-09-16 |
| NVFP4 prefill copies of the F16 GDN in_proj / gate / out_proj (`gemm.nvfp4_gdn_proj_prefill`) | `in` DEFAULT since 2026-09-17 (Qwen3.6-35B `in` alone pp4096 +7.7% 3/3; Qwen3.8-27B BF16-in_proj export +12.3% 3/3, PPL +0.56%); the MXFP8 twin `gemm.mxfp8_gdn_proj_prefill` is PPL-free (Open 12 row) but opt-in on VRAM | Qwen3.6-35B-A3B-NVFP4, `imp-cli --bench`, 3 alternating pairs: `in,gate` pp4096 146.66 / 147.18 / 146.18 -> 127.06 / 127.60 / 128.42 ms (+15.4%, 3/3); `all` 146.76 / 147.87 / 147.42 -> 121.63 / 122.87 / 121.37 ms (+20.9%, 3/3); pp512 under nsys (`in,gate`, 3 pairs): cuBLAS class per run 92.8 / 104.4 / 81.9 -> 42.9 / 44.1 / 46.0 ms, CUTLASS 80.4 / 80.1 / 81.6 -> 86.8 / 86.5 / 85.5 ms. PPL judge (deterministic, `ppl_corpus_45k`): 6.8543 -> in 6.9007, gate 6.8956, in+gate 6.9781, out 6.9955, all 7.1458 (+0.60..+4.25%). Copies 135 MiB per projection set of 30 layers, unbudgeted like the FP8 sidecar. Trap: `gemm_via_handle_` returns for tier-Undefined handles (ssm_in, gdn_gate: no cache entry) before the tier split, the route sits ahead of it | 2026-09-16 |
...
| transcript snapshot: the recurrent state at the END of every reply (unaligned position, partial tail block cached with it), `server.transcript_snapshot` default on | SHIPPED | the SWA path saved its window at finish, the hybrid path saved only at the prompt boundary, so every next turn re-prefilled the whole reply. Qwen3.8-27B-NVFP4-vllm, 1 stream, 5 turns, reasoning resent: cached_tokens 285/863/1195/2595 of 310/888/1223/2620 (turns 2-5) vs 0/304/880/1152; turn-5 TTFT 156 -> 49 ms. Needs the reply re-rendered token-exact: a forced think end now emits "\n" before `</think>` and `reasoning_content` reaches the template. The 8-concurrent-thinking-sessions `</think>` re-emit seen here (4-5 of 8 rows) was the batched sampler's shared ban buffer, CLOSED by #2026 (0 of 8 rows) | `transcript_snapshot_key`, `KVCacheManager::clone_held_block`, `HybridRestoreChainTest.TranscriptRestoreStateStaysClose` |
...
| (f) 3-D stacked experts | REFUSED (2026-08-01), SPLIT PER EXPERT (2026-09-15, unmeasured) | the old refusal never fired (#1188: experts copied through as BF16 while `hf_quant_config.json` announced NVFP4). De-stacking rejected against gpt-oss-20b: the fused layout is not one layout and expert biases have no per-expert loader path. 2026-09-15: the layout is a per-model_type descriptor (gpt-oss [ne, K, N] interleaved, Gemma-4 [ne, N, K] concatenated), the biases pass through to the loader's existing de-interleave. Open item 7 |
