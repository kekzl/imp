...
- **Not a CPU engine — with one deliberate exception under evaluation.** The forward pass is GPU-only: no AVX kernels on the dense path, no Metal, no Vulkan. The exception is **cold MoE experts computed host-side**, so that an 80B–120B-class MoE with a small active set fits this card at all ([`roadmap.md`](roadmap.md), "CPU-resident cold experts"). It is scoped deliberately narrowly — routed experts the current token does not touch, nothing else — and it is **not committed**: the gate is a measured bandwidth-and-latency budget on this host, not how attractive the idea sounds. If that budget says the CPU half cannot keep up, this line goes back to reading "GPU only" and the entry is shelved with the measurement attached.
