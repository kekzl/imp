...
>
> **This does NOT generalise to Gemma-4** (2026-08-02 audit, F-16). Its global layers are hd=512 and take `attention_cublas_prefill`, or `attention_cublas_prefill_sliced` (#1036) when the S-matrix overflows, *by design and by measurement*: both beat the SMEM-capped fused hd=512 kernel ([`docs/audit/gemma4_attn_routing_2026_07_16/PERF_LOG.md`](../audit/gemma4_attn_routing_2026_07_16/PERF_LOG.md)). On an advertised model the "legacy" path is the *default* prefill path for half the layer stack, and the 384 MiB `attn_scores` workspace is retained for it.
...
Learned sinks (gpt-oss) are pre-gated at `attention_dispatch.cu:45`: they route straight to the FP16 WMMA FMHA (the only sink-capable tier) and **throw** on decline rather than falling through to a sink-blind kernel (#992).
...
The decode dispatch (further down in `executor_attention.cu`) is a single `switch` on the KV cache dtype. The switch selects a **launcher**, not a kernel; each launcher picks among its own variants by batch, GQA ratio and split-K decision, so each row below is one dtype and up to four kernels (#1677 - this table named one kernel per dtype, and the FP16 row named a symbol that does not exist).
