...
| Sparse decode attention (top-k page selection) | ✅ opt-in | `attention.sparse_topk_tokens`: decode reads only the top-scoring KV blocks, bit-identical at ctx <= budget. Page score defaults to the page mean plus its standard deviation (`attention.sparse_score_meanstd`); the Quest min/max corner bound is the opt-out. F16/FP8 KV; gates in [`plans/2026-08-28-sparse-decode-attention.md`](plans/2026-08-28-sparse-decode-attention.md) |
