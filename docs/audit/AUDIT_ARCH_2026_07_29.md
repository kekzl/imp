...
5. **The two dispatch-level surprises.** *Good:* zero stale-target debt — all 24 mentions of
   ...
   here, and real sm_120a idiom (TMA + mbarrier pipelines, `mxf4nvf4`, green contexts) is
...
| **CUDA graphs** | OK | OK (bucketed by `n_sequences-1`) | separate gate `runtime.no_vision_graph` | OK for pure SSM too since 2026-08-12 (the reject was unverified; see the Nemotron row) |
...
| **PDL** | 34 | `runtime/pdl.h`, `cudaLaunchAttributeProgrammaticStreamSerialization`. Counted as idiom in this audit's summary rows; that was wrong and is corrected above. The device half is absent (`griddepcontrol: 0` in this audit's own `sm120.txt`), so the attribute cannot produce the overlap it exists for (#1655) |
