...
| 3 | F2-1 = F1-4, F2-3 + E-5, F2-2, E-6 | ✅ CLOSED (F2-2 documented at the proxy) | dispatch #3 PR | `tests/test_model_name_policy.cpp` 9 cases; mock battery 48 passed with the new path-shape and `/ready` cases; real binary: model-less `/ready` 503 `no_model` while `/health` 200, path-shaped names 503 envelope; with Qwen3-4B loaded + `--models-dir`: `/models/Qwen3-8B-Q8_0.gguf` 404 (pre-fix: swap), loaded basename 200, `/ready` 200, SIGTERM exit 0 | the burst-of-100-during-swap test (needs two checkpoints in the server lane); a `/ready` observation mid-swap on the real binary |
