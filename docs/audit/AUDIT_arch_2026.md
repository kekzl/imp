...
| 8 | A1-1, A1-2, A1-4, A1-6, A1-8, A1-5, A1-7 | ✅ CLOSED (A1-1 decision (b) retire; A1-2 decision keep + date; A1-4 2 of 3, `gemv_pdl_register` -> #11; A1-7 prefill half) | #NNNN | `GemmKernelRegistryTest.RegistryHoldsExactlyTheProducedKeys` (size 10, the 9 retired keys NoMatch) red with one registration re-added; `AttentionDispatchRules.PagedDecodeRefusesMxfp4KvAtHeadDim96` red with the case removed; `GgufWrongMIsOne` had been answered by the dead Q4_K-IMMA key (now NoMatch); test-compute registry 15/15, test-attention `*Paged*` 54/54, test-core dispatch 9/9, `make dev-test` 11/11; -2645 / +137 lines over 32 files; static gates green (lanes pin 1094 -> 1061, `gemm_cutlass_mxfp4_sm120.cu` alloc budget 11 -> 9, two `[allow]` entries dropped) | the (a) migration; a `MoeDecodePath` enum; `gemv_pdl_register` (#11); a real-binary `kv_cache.dtype=mxfp4` + hd=96 init refusal (asserted at the rule, not observed); the perf gate stays the single-arm push hook (#10) |
