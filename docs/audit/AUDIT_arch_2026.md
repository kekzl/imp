...
| 17 | B-4 (no P3 row: the queue closed without one) | ✅ CLOSED | #1939 | `make check-alloc-interpose` in two phases: A (residual KV + MTP, the old config) 19 -> 1 pinned call; B (`mtp_k=0`, four 1.5k-token prompts at once + three json_object, FATAL without a `Ragged prefill: [2-9] seqs` and a `ConstrainedPipeline: launched` line) 3 pinned calls, 0 `cudaMallocAsync`, 0 pinned-host; `tests/test_serving_metadata_layout.cpp` 4 tests in the unit lane, mutant "rounding dropped" fails `Aligned` and `Disjoint`; alloc allowlist 477 -> 466 sites; `make verify-fast` green on the code tree c79163fc (tg128 292.8 vs pin 287.2, pp4096 -2.35 %, peak +0.62 %) | `s_norm_scratch` (`engine_spec_mtp.cpp`, 0.22 MiB, regrown per feed length) and `JsonConstrainer::init` (once per process) stay pinned; `chunk_eager_k_/_v_` fired in neither phase |
