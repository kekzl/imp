...
| P5 | dispatch log, one row per dispatch as the P3 queue was worked; closed 2026-09-08 |
...
| 2 | F1-1, F1-2, F1-5, F1-7, F1-11, F1-12 | ✅ CLOSED (F1-12 partly: 8 of 10 parsers fuzzed) | #1910 | `GgufFaultInjection` 20 -> 25 cases, all green; `FuzzCorpus.GgufLoader` + `MmprojLoader` 250 iterations each; F1-7 mutant `FAILED`; F1-1 mutants refused on both trees in the plain build (the write is stack-only), ASan run recorded in the PR | the two hostile-file cases the P3 gate names are in the corpus and the battery; `config.json`, Jinja, SentencePiece, warm-cache and LoRA fuzz targets; a split-GGUF truncation fixture |
| 3 | F2-1 = F1-4, F2-3 + E-5, F2-2, E-6 | ✅ CLOSED (F2-2 documented at the proxy) | #1912 | `tests/test_model_name_policy.cpp` 9 cases; mock battery 48 passed with the new path-shape and `/ready` cases; real binary: model-less `/ready` 503 `no_model` while `/health` 200, path-shaped names 503 envelope; with Qwen3-4B loaded + `--models-dir`: `/models/Qwen3-8B-Q8_0.gguf` 404 (pre-fix: swap), loaded basename 200, `/ready` 200, SIGTERM exit 0 | the burst-of-100-during-swap test (needs two checkpoints in the server lane); a `/ready` observation mid-swap on the real binary |
| 4 | I-1 = D-3, I-3, I-7, I-8, H-1, H-4, G-6 | ✅ CLOSED (I-8 documented, H-4 doc-fixed, no new release stage) | #1911 | `guard_lane_filter_copy` + `guard_makefile_filters` in the unit lane, both red on their mutant (stale copy; the old `test-text` line); 17 Makefile patterns resolve in the container; `check_function_size.py` selftest 12/12, the 556-LOC body surfaces and is allowlisted; `deps` gate green offline; new `make test-e2e` containers: 11 green on first run, the greedy lock RED on the `17 + 25` row of both checkpoints (moved after token 3 to llama.cpp's 06-04 continuation, answer unchanged), rows regenerated | `verify-north-star` / `verify-chunked` still run from no hook; the Qwen3-4B lock row was not generated (the Qwen3-8B rows are used instead); which PR moved the greedy trajectory (nothing ran the lock for 3 months); the FA2 family stays outside `verify-fast` (I-2, not in this dispatch) |
| 5 | D-1 | ✅ CLOSED | #1913 | `CudaFaultSignalTest` x2 (real illegal address, threadsafe death test, 316 ms) + `DeviceFaultSignalTest` (Qwen3-4B-Instruct-2507-Q8_0, 3.3 s) green; mutant with the executor throws and the per-step probe removed: FAILED, child "in-flight request finished with length"; `make dev-test` 11/11; the three deliberate clears (`engine_workspace_warmup.cpp`, `cuda_graph.cu`, `graph_diag.h`) untouched and their classes pinned recoverable | a fault injected into a running `imp-server` binary (no debug hook; the contract is tested at the `BatchingEngine` level); `/ready` reads `faulted()` with no real-binary observation |
| 6 | E-1, F1-6, E-9 | ✅ CLOSED | #1914 | `LoraHotSwap` x3 on Llama-3.2-3B green (1659 / 659 / 937 ms), `LoraDimsTest` x3 in the unit lane; mutant "adapter fold dropped": B reuses 160 tokens of A's KV, FAILED; mutant "barrier dropped": `decode_batch_max` 2, FAILED; K+64 / N+128 / [2^20, 2^20]-over-64-bytes adapters refused `IMP_ERROR_INVALID_MODEL`; `make test-e2e` runs `LoraHotSwap.*` (ran from no target before) | a real-server two-client mixed-adapter run (the contract is tested at the `BatchingEngine` level); FFN adapters on MoE models are validated against `d_ff` but the executor applies FFN deltas on the dense path only (pre-existing) |
| 7 | C-1, C-6, C-5, C-9 | ✅ CLOSED | #1915 | `ServingSignalsTest` x3 on Qwen3-4B green (3844 / 1053 / 1672 ms): mid-decode exhaustion finishes `capacity` with `KvCapacity`, running/waiting 1/1 while the first request generates and the second's `queue_ms` > 100 ms, 1 verify / 64 drafted / 62 accepted on the request; mutants red (decode-site reason dropped: "cancelled"; `queue_ms` closed from `t_submit`: reads 0); `SpecUsage` x2 + two transform pass-through tests + the Responses knob test in the unit lane | a `/metrics` scrape under a burst on the real binary; `/v1/completions` streaming usage untested on its own |
| 8 | A1-1, A1-2, A1-4, A1-6, A1-8, A1-5, A1-7 | ✅ CLOSED (A1-1 decision (b) retire; A1-2 decision keep + date; A1-4 2 of 3, `gemv_pdl_register` -> #11; A1-7 prefill half) | #1916 | `GemmKernelRegistryTest.RegistryHoldsExactlyTheProducedKeys` (size 10, the 9 retired keys NoMatch) red with one registration re-added; `AttentionDispatchRules.PagedDecodeRefusesMxfp4KvAtHeadDim96` red with the case removed; `GgufWrongMIsOne` had been answered by the dead Q4_K-IMMA key (now NoMatch); test-compute registry 15/15, test-attention `*Paged*` 54/54, test-core dispatch 9/9, `make dev-test` 11/11; -2645 / +137 lines over 32 files; static gates green (lanes pin 1094 -> 1061, `gemm_cutlass_mxfp4_sm120.cu` alloc budget 11 -> 9, two `[allow]` entries dropped) | the (a) migration; a `MoeDecodePath` enum; `gemv_pdl_register` (#11); a real-binary `kv_cache.dtype=mxfp4` + hd=96 init refusal (asserted at the rule, not observed); the perf gate stays the single-arm push hook (#10) |
| 15 | G-5, G-4, G-3, G-7, G-8, G-10, G-12 | ✅ CLOSED (G-3 partly: contract test, the no-consumer fact stands; G-7 deleted, not wired) | #1917 | `CApiContract` x3, `RuntimeConfigTest.MaxSeqLenBindsAndYieldsToTheCliValue`, `GgufFaultInjection.Q8_1WeightTypeIsRefusedAtParse` green in the unit lane (`make dev-test` 11/11); the three code mutants (validation dropped, key wins over the flag, refusal dropped) FAILED in one build, restore green; `check_api_guard.py` 23/23, 22/23 with the guard token removed from `imp_gpu_release`, selftest 10/10; `moe_fp32_down 352.0 MiB` measured on `ee03cfe9` (Gemma-4-26B-A4B-it-UD-Q4_K_M, `--max-seq-len 4096`), gone on this tree by construction; -570 lines over 31 files | wiring the six Gemma-4 flags to config keys (no writer since #319, no measurement asked for them, so they went); an external C-API consumer (not checked); `make verify` full lane |
| 10 | H-3, H-2, H-9 | ✅ CLOSED (H-2 documented + hook reminder, not automated: no GPU runner; H-9 by threshold + reason, the guarded re-pin refuses today's top-range readings) | #1918 | `make verify-ab` on `eb00defe` built twice, 6 gate runs x 3 pairs: paired decode means +0.20 / +0.05 / -0.17 / -0.05 / -0.03 / -0.13 %, single pairs -0.78..+0.48 %, 0 red at the 2 % threshold; planted regression (a second o_proj GEMV on every even layer): paired -2.93 % (3/3 negative) = FAIL, single-arm 284.17 tok/s = -1.05 % vs the pin = PASS; seven kernel-knob probes before it moved the pair -0.06..-0.56 % and `__launch_bounds__(kKparThreads, 16)` read +1.88 % (lever, recorded); one gate run = 3 pairs = 6 processes in ~37 s, the base image 3.5 min once per main sha | re-pin of the single-arm baseline (refused: 291-294 vs the 275-290 healthy band of `repin_baselines_if_median.sh`); a roofline run on kernel diffs (needs the declined GPU runner); `verify-ab` on the north-star / chunked pins (same script, `IMP_VERIFY_BASELINE`, not wired into the hook) |
| 13 | J-1..J-12, C-7, C-8, A2-6, A2-7, D-4, E-9, I-9, P0-1, P0-4 | ✅ CLOSED (E-9 already closed on `main` by #1914 + the ledger rewrite, the registry-flag item by #1916; J-11 by a FEATURES note, no Mixtral checkpoint run; J-3 fixed, the content-checking citation gate left out) | #1919 | `docs_lint` 0 errors (40 pre-existing warnings), `check_doc_citations` 0 dead across 32 living docs, `sync_docs --check` clean; `tools/check_config_keys.py` 224/224 keys both directions, selftest 5/5; `tools/check_changelog_form.py` 25 `[Unreleased]` entries, 0 over 3 lines, selftest 7/7; `docs_lint` path-derived layer 0 mismatches after the `vision_gemma4v_spec.md` move (it was the one L2-in-`docs/` case); `make build` green; pre-commit `make test-gpu` 2695 passed, 0 failed, 106 skipped (model-gated); pre-push `verify-fast` + `verify-ab` tg128 279.28 (-2.75 % vs pin 287.19, host at ~275-279 this evening on both arms) / pp512 12066.77 (-2.74 %) / pp4096 15206.79 (-0.77 %) / peak 20762 MiB (+0.22 %) all PASS; verify-ab vs `imp:ab-4d0da33d`, 3 pairs: paired decode mean +0.18 % (+0.16 / -0.78 / +1.16), paired prefill mean +1.26 %, PASS at the 2 % threshold | a content-checking citation gate (symbol-anchored citations, ~80 LOC + a 33-doc sweep); a Mixtral checkpoint row (nobody ran one); CHANGELOG sections older than 0.37.0 keep their long entries (records); the two dead includes P0-1 / P0-4 re-pin (removal is dispatch #14) |
| 9 | H-7, H-8 (attribution half) | ✅ CLOSED (H-8 pin half stays open, no P3 row) | #1920 | `bash scripts/ci_static_gates.sh hygiene docs citations` green with the new "license" clause; `make build` green after removing `LICENSE` from `.dockerignore` (the COPY failed with "/LICENSE": not found first); `ls /usr/share/doc/imp` in `imp:test`: `LICENSE` 1062 B + `THIRD_PARTY_LICENSES.md` 13617 B, label reads `MIT AND Apache-2.0`; mutant: file removed -> "license" section FAIL | SHA pins for the cmake deps, Dockerfile clones and actions (H-8 pin half); the in-file header still says "thu-ml/SageAttention3" where the repository is `thu-ml/SageAttention` (subtree `sageattention3_blackwell`): comment-only `.cu` edit deferred to the next GPU-free code PR, the licence file names the correct path |
| 14 | P0-1..P0-5, G-1, G-2, G-9 | ✅ CLOSED (P0-2 and P0-5 priced and pinned, not moved; the layer SCC stays 8 nodes as the scout's simulation predicted) | #1922 | `tools/check_layering.py`: 1729 forward / 24 backward lines over 13 pinned edges, 0 violations, selftest 5/5; `rg -c 'runtime/config.h' src/exec` = 0; `compute/quant/model/memory -> runtime` = 0; reverse-BFS TUs (src+tests+tools): `runtime/vram_budget.h` 82 -> 53, `model/model_config.h` 159 -> 148, `executor.h` 80 unchanged; `GraphExecutor::runtime_config()` -> `dispatch_policy()` 143 sites; mutant (a `runtime/config.h` include planted back into `src/exec/`) -> UNPINNED fail; `make dev-test` 11/11 green; `make build` green (589 steps); GPU suites not run (card held by another tenant, CI has no GPU) | `CudaGraphRunner` stays in `runtime/cuda_graph.cu` (vision -> runtime = 1 line, pinned); `model -> exec` 1 line (pinned); the residual `memory <-> model`, `model <-> vision`, `compute -> model -> quant -> compute` cycles are ordering decisions and pinned per header; H-8 SHA-pin half still has no row |
| 11 | A2-1 = A1-3, A2-2, A2-5, A2-3, A2-4 | ✅ CLOSED (A2-4 priced and deferred: FP8 KV at hd=256 is an explicit opt-in and the port is a lane-layout change; the `_mb_` FP16 / multirow kernels are test oracles and stay unregistered) | #1923 | carveout: Qwen3-8B-Q8_0 with the NVFP4 decode cache off, tg128 145.3 -> 157.3, +8.24 / +8.20 / +8.25 % (3/3); batched LM-head GEMV registered: @32 Qwen3.8-27B +1.49 / +1.20 / -0.18 % (mean +0.84 %); producer triggers in 24 paged kernels: @32 +0.84 / -0.40 / -1.23 % (mean -0.26 %), M=1 -0.02 %, NEUTRAL; single-sequence GDN SPLIT=2: M=1 +0.12 / +0.20 / +0.12 % (3/3), PPL 4.6242 -> 4.6242, the 4 spilling instances leave the resource baseline; the 7 NVFP4 K-par GEMVs drop their 12-CTA `__launch_bounds__` min-blocks term (the #10 lead spelled it `(128, 16)`: infeasible on the 1536-thread SM, same SASS as no term; ptxas 40 -> 42 registers): Qwen3-8B-Q8_0 +1.96 / +1.97 / +1.71 %, Qwen3-14B-NVFP4 +0.65 / +0.61 / +0.79 %, @32 Qwen3.8-27B +1.19 / +2.12 / +0.08 % (3/3 each); `GreedyDeterminism`, `GDNModelTest.*`, `GDNScanTest.*`, `*Paged*` green; `kernel_resource_baseline.txt` re-pinned | the A2-4 port; the two split-K reduce kernels carry no trigger; `gemm_nvfp4_batched` / `_acc` are test-only exports (A1-4 class, kept as the smallm oracle) |
| 12 | D-2, D-5, A1-1 (numerics half) | ✅ CLOSED (A1-1's numerics half was already gone with #1916: the registry holds 10 keys and no FP8 entry. E5M2 keeps its load-time refusal, `Fp8Source.AcceptsTheDtypeSpellingsExportersUse`, rather than a golden) | #1924 | the number D-2 asked for, on Qwen3-14B-NVFP4 teacher-forced at 24+64 tokens, `IMP_DETERMINISTIC=1`: M=1 vs M=32 moves mean NLL 3.440330 -> 3.488856 and flips 7 of 64 greedy tokens (mean \|dlogp\| 0.242, max 1.636); `gemm.nvfp4_smallm=false` reads 0.261 / 7 flips, i.e. the activation format is NOT the term, the batch shape is; both M=1/M=1 control arms are bit-exact (0.000e+00), so the delta is attributable. D-5: 6 dequant goldens added (Q4_1, Q5_0, Q5_1, Q2_K, Q3_K, Q8_K) + dp4a GEMV goldens for Q2_K/Q3_K + a two-way coverage gate; **Q3_K was broken in both its paths** (hmask plane read as byte `i/8` bit `i%8` instead of byte `i%32` bit `i/32`, max rel 4.0 vs the ggml reference, ~half of every weight off by 4 scale steps) and is fixed. 4 mutants, 4 red: Q3_K dequant reverted -> `GgufDequant/6` red; Q3_K trait reverted -> `GgufRef.Q3_K_GemvDp4a` red (Q2_K stays green); a format row deleted -> coverage gate red; a 2 % scale error in the smallM A4 kernel -> the pinned-NLL anchor red (every DELTA arm stayed inside its fence, which is why the anchor exists) | the seven "untested" LIMITATIONS formats keep no e2e checkpoint gate, only kernel-level goldens; `docs/PERF.md` records one model, not a sweep; the M=1 branch declining a weight to the `M <= 32` smallM gate is documented, not changed |
| 9b | H-8 (pin half; the P3 queue closed without a row for it) | ✅ CLOSED | #1926 | `check_dep_pins.sh` green offline and `--online` on the unchanged tree; 5 planted mutants, 5 red: cmake SHA drifted from the Dockerfile default, Dockerfile tag drifted, a dep with a tag and no SHA, one `uses:` back on `@v7`, and a tag that no longer resolves to its pin (the re-tag alarm, `v3.11.3` -> `9cca280a4d0c...`). `make build` green, 590/590 steps, with the deps layer rebuilt as `git fetch --depth=1 <sha>`; `/deps/PINS` in the toolchain image carries url/tag/sha for all four. Depth-1 SHA fetch measured against all four remotes (252 / 7787 / 337 / 1163 files) and `GIT_SHALLOW TRUE` with a hash under the pinned CMake 4.3.1 (1-commit checkout), so the pin costs no clone time. 35 `uses:` lines are 40-hex with the release as a comment | H-6, the same class one layer down (the `cmake.sh` download has no `sha256sum`, base images are tag- not digest-pinned, `apt-get` is unversioned): still no row. `--require-hashes` for `tests/api/requirements.txt` needs the transitive set. Dependabot's rewrite of a SHA pin is documented, not yet observed on this repo |
| 16 | H-6 (no P3 row: the queue closed without one, like 9b) | ✅ CLOSED (apt-get left unversioned with a reason; the four `tools/Dockerfile.*` developer harnesses are out of scope, named in the gate) | #1927 | `check_dep_pins.sh` green offline and `--online`, `--selftest` 10/10 over a fixture tree (wired into `ci_static_gates.sh deps`); 9 planted mutants on the real tree, 9 red: base image back on a tag (Dockerfile and Dockerfile.plot), the ci.yml copy drifting to a second digest for the same tag, the cmake checksum deleted, `git-clang-format` back on `release/18.x`, a checksum naming a different path than the download, a requirement with no `--hash`, a lock pin loosened to `>=`, `pip install matplotlib==3.11.0` without a lock file. The CMake hash was verified by downloading the 64,025,554-byte installer and matching Kitware's `cmake-4.3.1-SHA-256.txt` (`85947732c8eb85fb...`). The API lock file installs under both CI pythons: 3.12 on the host runner and 3.14 in the CUDA container (venv, `pytest --collect-only` 144 / 67 nomodel, same counts as before); `Dockerfile.plot` builds from its 11-requirement lock file | the four developer-harness Dockerfiles (`tools/Dockerfile.{agents,agents-sdk,claude-code,ncu}`, `tools/analysis/*`) still pull tag-pinned images and unpinned pip packages, on one operator box; `apt-get` versions; base-image digests have no automated refresh (Dependabot's docker ecosystem rewrites them, unobserved on this repo); model downloads in `scripts/stage-model.sh` |
| 17 | B-4 (no P3 row: the queue closed without one) | ✅ CLOSED | #1939 | `make check-alloc-interpose` in two phases: A (residual KV + MTP, the old config) 19 -> 1 pinned call; B (`mtp_k=0`, four 1.5k-token prompts at once + three json_object, FATAL without a `Ragged prefill: [2-9] seqs` and a `ConstrainedPipeline: launched` line) 3 pinned calls, 0 `cudaMallocAsync`, 0 pinned-host; `tests/test_serving_metadata_layout.cpp` 4 tests in the unit lane, mutant "rounding dropped" fails `Aligned` and `Disjoint`; alloc allowlist 477 -> 466 sites; `make verify-fast` green on the code tree c79163fc (tg128 292.8 vs pin 287.2, pp4096 -2.35 %, peak +0.62 %) | `s_norm_scratch` (`engine_spec_mtp.cpp`, 0.22 MiB, regrown per feed length) closed by #1940 (phase A pin 0, workspace field sized at enable time); `JsonConstrainer::init` (once per process) stays pinned in phase B; `chunk_eager_k_/_v_` fired in neither phase |
| 18 | B-5, B-6, B-7 (no P3 rows: B-5 was "open question until an instrument exists", B-6/B-7 unqueued) | ✅ CLOSED (B-5 instrument + key; the 16/32/64 sweep is the PERF_LOG entry) | #1941 | `kv_cache.block_size` binds (`RuntimeConfigTest.KvBlockSizeBindsAndDefaultsToAuto`), `kv_block_size_error()` serves 16/32/48/64/128/256 and refuses 1/8/15/24/100/272/512 (`KvBlockSize` x2), `ExecT2Demand.SplitkReadsTheResolvedKvBlockSize` (32 -> half the 16-block term); `KVCacheTest.ResidencyProbe*` x3 in test-kv: a 256 MiB pool reads 1287-1288 GB/s, mapped pinned host memory 130-139 GB/s over the same routine (the falsifier: host-resident memory reads as spilled, threshold 500); `make dev-test` 12/12; `verify-fast` green (tg128 281.26 vs pin 287.19, -2.06 %; pp512 -0.61 %; pp4096 -0.27 %; peak +0.62 %); `kv_block_size_ab.sh 16 32`: 32 never ahead, Qwen3-8B-Q8_0 -1.80 / -0.89 / -0.01 % at 2k / 8k / 32k, Qwen3.8-27B-NVFP4 -4.32 / -1.51 / -1.50 %, so **auto is 16 for every model** (the 2026-03-23 rule REFUTED); the shipped path probes 1554 GB/s on Qwen3.8's pool | `bitdecoding_qk` (opt-in TC path, `block_size <= 16` WMMA mapping) not re-validated; weights and caches are not probed, only the KV pool; 64 measured at kernel level only (`imp-bench decode-attn`), not e2e |
| 19 | F1-3, F1-8, F1-9, F1-10, F2-4, F2-5, F2-8, F2-9 (no P3 rows: the queue closed with 29 findings unassigned, these eight are the input-hardening class and hold the last S1) | ✅ CLOSED (F2-9 `--max-concurrent` half left as the finding left it: tokenize does no GPU work) | #1942 | `make dev-test` 12/12 with 9 new CPU tests (`WeightRecordIndicesTest` 11 mutations of one record, `SentencePieceLoader.RejectsLengthDelimThatWrapsThePointer`, `CheckpointLimits.RecursiveMacro*` / `HugeLoops*` / `MalformedJsonNumbers*` / `JsonNodeBudget*` / `DeclaredPerDimensionCounts*` / `AConfigDeclaringAGiantHeadDim*`); model-less API lane on the built `imp-server` 105 passed with `TestPerRequestCaps` x5 (n 1000, 20000 messages, 5000 `logit_bias`, 9 images, 1000 rerank documents all 400), mock lane 54 passed; 7 mutants, 7 red: SentencePiece guard reverted FAILED, `CallDepthGuard` removed SIGSEGV, `count_iteration()` removed FAILED, `data_alloc` range check removed SIGSEGV, `from_chars` error ignored FAILED, node budget removed FAILED, image cap removed = real binary 503 not 400; static gates green after re-pins (`hf_config_loader.cpp` 963 -> 976, `jinja.cpp` 2407 -> 2454, `load_config` 543 -> 555, lanes 1071 -> 1072), `kernel-resources` 72/878 match; `make build` green; `test-e2e WarmCacheTest.*` 3/3 on Qwen3-8B-Q8_0, the hostile record refused in the log as `data_alloc 2147483647 outside 1 allocs`; pre-push `verify-fast` PASS (tg128 294.58 vs pin 287.19 +2.57 %, pp512 +1.22 %, pp4096 +2.63 %, peak VRAM 20844 MiB +0.62 %) and `verify-ab` vs `imp:ab-b018362f` 3 pairs: paired decode mean +0.02 % (+0.13 / -0.03 / -0.05), prefill +0.16 % | a `config.json` fuzz target, a Jinja render fuzz target and a warm-cache fuzz target (F1-12's open list) still absent; the warm-cache `/tmp` fallback when `HOME` is unset unchanged; `--max-concurrent` still does not cover `/tokenize` |
| 20 | C-2, C-3, C-4, C-10 (no P3 rows: the scheduler / graph class of the 29 unassigned findings) | ✅ CLOSED (C-3 recovers only while nothing was evicted, by design; C-10 resolve-and-say-so, the F16 recapture-per-chunk-length stays) | #1943 | `make dev-test` 12/12 with 3 new CPU tests (`GraphEligibility.KvPressureCountsReclaimableBlocksAsFree`, `KvPressureLiftsAtAFifthUnlessSomethingWasEvicted`, `SchedulerTest.AnAgedRequestHoldsTheQueueUntilItsBlocksAreFree`); 3 mutants, 3 red (reclaimable term dropped, eviction pin dropped, aged hold removed); `test-e2e ServingSignalsTest.*` 4/4 on Qwen3-8B-Q8_0 with F16 KV pinned: the log reads `KV cache >90% full (2 free + 0 reclaimable of 35 blocks) - auto-enabling StreamingLLM`, `CUDA graphs disabled: streaming_kv_kv_pressure (mid-run, lifted when the pressure clears)`, then `KV cache pressure cleared (34 free + 0 reclaimable of 35 blocks, nothing evicted)`, `CUDA graphs re-enabled`, and the next request decodes on the replayed graphs; C-10 on `imp-cli --set kv_cache.dtype=fp8`: `prefill graph: off, the FP8_E4M3 KV append syncs its scale to the host per chunk and cannot be captured`, and the same line fires at defaults on Qwen3-8B because the resolver's "auto" picks FP8 KV for Qwen3 (the finding's falsifier does not hold for the flagship class); static gates green after re-pins (`engine_scheduler.cpp` 1331 -> 1358, test-lanes 1072 -> 1073, the e2e lane guard's frozen list +1), `kernel-resources` 72/878; pre-push `verify-fast` PASS (tg128 300.00 vs pin 287.19 +4.46 %, pp512 +2.14 %, pp4096 +3.28 %, peak VRAM 20844 MiB +0.62 %), `verify-ab` vs `imp:ab-2a968200` 3 pairs: paired decode mean +0.04 % (0.00 / +0.17 / -0.04), prefill -0.03 % | the throughput cost of the aged hold on a tight pool is not measured; re-promotion after an eviction stays impossible (the evicted KV is gone); keying the prefill graph runner by chunk length is a design change; the `--kv-fp8`-class CLI flags and the C API pin the dtype before the resolver, so the C-10 line does not fire for them (the flag itself is still resolved off) |
| 21 | E-2, E-3, E-4, E-7, E-8 (no P3 rows: the server class of the 29 unassigned findings) | ✅ CLOSED (E-3 documented + pinned as `processed_logprobs`, not snapshotted raw; E-4 priced and made a knob, the hold itself stays; E-7 `endpoint` + `model`, no `status` label: the 4xx/5xx split already exists; E-8 verified, the digest stays unkeyed) | #1944 | `make dev-test` 12/12 with `InflightGate` x2 (64 threads race 8 slots) and `RuntimeConfigTest.AgentScanLimitBindsAndDefaultsTo256`; model-less API lane on the built binary 108 passed (`TestThinkingSignature` x2, `test_metrics_series_carry_the_endpoint_label`), mock lane 55 passed; 2 mutants, 2 red (gate limit check removed: both `InflightGate` tests FAILED; signature check removed: the edited block reads 503 instead of 400); `scripts/test_server.sh` on Qwen3-8B-NVFP4-cortecs: every battery PASS, the logprobs gate ran the `logit_bias -100` case (the biased greedy token leaves the top-k, top-1 == chosen), degen suite 50 checks 0 FAIL; static gates green (`handlers_chat_core.cpp` and `handlers_chat.cpp` back under 800 after the eight observe sites folded into `ServerMetrics::record_*`), `kernel-resources` 72/878; pre-push `verify-fast` PASS (tg128 297.68 vs pin 287.19 +3.65 %, pp512 +2.27 %, pp4096 +2.96 %, peak VRAM 20844 MiB +0.62 %), `verify-ab` 3 pairs: paired decode mean -0.09 % (+0.04 / -0.48 / +0.18), prefill -0.24 % | the "+8" workers are still not reserved for `/health` (a second listener); httplib's `mqr` closes the connection with no status, which is the upstream semantics; the 256-token hold on tool requests is unchanged by default; the signature stays a plain FNV digest (no SHA-256 in tree, documented); the Grafana dashboard has no panel on the new series yet |
| 22 | I-2, I-4, I-5, I-6, F2-6, F2-7, H-5 (no P3 rows: the test-infrastructure class of the 29 unassigned findings) | ✅ CLOSED (I-2 by the default-path fixtures, not the family; F2-7 recorded, not fixed: it needs a native-Linux GPU box) | #1945 | `Fmha*:Fhma*` measured on the 5090: 170 tests in 5m19s (FmhaSm120Test 24 tests 120.0 s, FmhaFP8Test 11 tests 104.7 s, FmhaFA2Dense2CtaTest 5 tests 39.8 s, FmhaFA2Test 36 tests 20.7 s, FmhaFA2Hd256Bkv32Test 12.5 s, FmhaFA2Hd256Test 11.9 s, FmhaFA2PvF16Test 6.5 s, FmhaHd512Test 1.0 s, FmhaFA2Fp8ScaledTest 0.3 s, FmhaFA2HeavyFirstTest 0.1 s, FhmaMxFP4Test 35 tests 0.1 s); the seven default-path fixtures (~40 s) join `verify-fast`, `guard_verify_filter` green; `SamplingAdvancedTest` x6 (test-compute) 6/6, mutants DRY threshold removed (1 FAILED) and mirostat mu update removed (2 FAILED); catalogue anchors M57-M66, `--verify-anchors` 66/66, `run.py --only M57..M66 --ci-only`: 10/10 KILLED (two by SIGSEGV: the data_alloc index and the Jinja call depth), score 100 %; `make test-quantize`: Qwen3-0.6B BF16 -> NVFP4 modelopt (196 tensors quantized, 115 copied, 870 MB), imp-cli on the result answers " Paris. The capital of France is also" at 561 tok/s decode; `make dev-test` 12/12; static gates green (lanes pin 1073 -> 1079), `kernel-resources` 72/878; the first pre-push run of the widened filter failed on `KVCacheTest.ResidencyProbeCoversTheCommittedPrefixOfAGrowablePool` reading 429.9 GB/s (its 512-block pool gave the probe a 32 MiB set, four launches' worth of latency, after 170 FA2 tests in the same process): the test commits 4096 of 8192 blocks now, 256 MiB of traffic; second run: `verify-fast` PASS with the widened filter (tg128 299.79 vs pin 287.19 +4.39 %, pp512 +2.85 %, pp4096 +3.54 %, peak VRAM 20844 MiB +0.62 %), `verify-ab` 3 pairs paired decode mean +0.06 % (+0.05 / +0.06 / +0.08), prefill +0.16 % | the FA2 legacy / FP8 / 2-CTA / bkv32 fixtures stay outside `verify-fast` with their numbers recorded; the nightly fuzz job has not had its first scheduled run; `scripts/validate_safetensors.py` still has no invocation site; the mutation harness stops on any dirty tree, so anchors run only between edits |
| 23 | A1-9, A2-8, A2-9, H-10 (no P3 rows: the last four of the 29 unassigned findings; with this row every finding of the audit has a verdict in SETTLED H) | ✅ CLOSED (A1-9 recorded, not retired; A2-8 observability, the path stays; H-10 measured NEUTRAL, the option stays off) | #1946 | `make dev-test` 12/12 with `RuntimeConfigTest.LmDequantBisectBindsUnderDiagnosticsOnly`; static gates green (the config-keys gate caught the moved key landing under `[ffn]` on the first try), `kernel-resources` 72/878; H-10: `IMP_DEVICE_LTO=ON` image built on the fourth attempt (nvcc refuses `-dlto` beside `-gencode`, wants `code=lto_120a`; the device-link genex needs `$<COMMA>`; every CUDA target needs `-rdc`), the verify-fast gtest filter green on it, `verify_ab.sh imp:test imp:lto` 5 alternating pairs on Qwen3-8B-Q8_0: paired decode mean 0.00 % (+0.03 / -0.01 / -0.61 / +0.03 / +0.56), paired prefill -0.41 % (-0.43 / -0.87 / -0.75 / -0.25 / +0.26); pre-push `verify-fast` PASS (tg128 299.78 vs pin 287.19 +4.38 %, pp512 +2.66 %, pp4096 +3.39 %, peak VRAM 20844 MiB +0.62 %), `verify-ab` 3 pairs paired decode mean +0.01 %, prefill -0.08 % | the MXFP4 attention family's ~2550 LOC stay in the tree until the owner retires them; no register/spill comparison of the LTO SASS (the pin reads the host build dir, not the image); the LTO build's compile time is not comparable (three of four attempts stopped mid-build and warmed the cache) |

# CAMPAIGN CLOSED (2026-09-08)

| | value |
|---|---|
| P5 rows | 24: dispatches 1-23 plus 9b (the P3 queue of 15, then 9b, 16 and 17 for findings the queue left without a row, then 18-23 for the 29 unassigned findings by class) |
| landed | #1909-#1920, #1922-#1924, #1926, #1927, #1939, #1941-#1946 (one PR per row) |
| open findings | 0: every ID has a row in `SETTLED.md` H or a P2 kill-list verdict |
| last row | dispatch 23, #1946, 2026-09-08 |

P5 takes no further rows. A new finding is a new audit, not a row here.
