...
| 9 | H-7, H-8 (attribution half) | ✅ CLOSED (H-8 pin half stays open, no P3 row) | # | `bash scripts/ci_static_gates.sh hygiene docs citations` green with the new "license" clause; `make build` green after removing `LICENSE` from `.dockerignore` (the COPY failed with "/LICENSE": not found first); `ls /usr/share/doc/imp` in `imp:test`: `LICENSE` 1062 B + `THIRD_PARTY_LICENSES.md` 13617 B, label reads `MIT AND Apache-2.0`; mutant: file removed -> "license" section FAIL | SHA pins for the cmake deps, Dockerfile clones and actions (H-8 pin half); the in-file header still says "thu-ml/SageAttention3" where the repository is `thu-ml/SageAttention` (subtree `sageattention3_blackwell`): comment-only `.cu` edit deferred to the next GPU-free code PR, the licence file names the correct path |
