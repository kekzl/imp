...
Resolution (2026-09-10): not server history but a race that any other tenant on the card opens. Same request with `logprobs`, compared run against run: idle card 0 of 38 runs differ; with a concurrent GPU tenant 3 of 39 runs differ from the first generated token on (the prefill output), 0 of 39 with a 1107-token prompt (a 3-token perturbation decays through the GDN state). `ssm_conv1d_prefill_kernel` / `_f32_silu_kernel` committed the conv window from the last row's block while the blocks of rows 0..K-2 read the previous window from the same buffer. Per-layer prefill dumps (`diagnostics.dump_hidden_dir=all`, graphs off, 30 requests under the tenant): the one perturbed request is bit-identical through layer 37 and differs at layer 38 (a GDN layer) in rows 1 and 2 only (max 0.0625 / 0.0247), the rows whose conv window comes from the previous state; everything after layer 38 differs. Commit moved to `ssm_conv1d_commit_kernel`, launched after the grid: 0 of 79 runs differ under the same tenant. Red herrings measured on the way, all under the same tenant. Perturbed runs per runs (`logprobs` compared run against run, 20 tokens): PDL off 3/39, FP16 KV 1/39, chunkpar scan off 3/79, FA2 off 3/36. Greedy flips per requests (260 tokens): FP8 KV 3/15, small-M GEMM off 1/15, n-gram off 1/7, graphs off 1/7. Greedy flip counts are not a bisection instrument: a config change moves the near-tie margins, so 0/22 arms (FP16 KV, PDL off) read as fixes and were not.
...
Closed 2026-09-11 (P3 follow-up): an in-think EOS was suppressed and kept in the context (engine_sampling_stop.cpp ~100-115); when the near-tie landed on `<|endoftext|>` the model continued as a new document. Now `think_logic::stop_mask_active` bans every stop id before sampling on the steps should_stop would suppress one: inside the think block while a budget can force `</think>`, and after the close until content appears (bounded by kMinAnswerAfterThink). Same rule on the eager sampler (`stop_mask_ids_` replaces the banned list), the conditional graph loop (`d_stop_mask_active` flag written by post_decode_step_kernel, `ban_logits_if_kernel`), the constrained pipeline and the n-gram verify. Reproduction `EndToEndModelTest.InThinkStopMaskKeepsStopTokensOutOfTheContext` (Qwen3.5-4B-mxfp4, logit_bias +100 on every stop id): before 17 x `<|im_end|>` and no `</think>`, after `</think>` plus answer content and one trailing stop.
