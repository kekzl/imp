...
|---|---|---|---|
| B-1 `logit_bias` arena slots never re-armed | FIXED — `sampling_cleanup_bias()` + a registered hook in `sampling_penalties.cu`; `SamplingTest.LogitBiasRearmsAfterStaticReset` fails without the hook (arena `used()` does not move on the second preallocate) | `src/compute/sampling_penalties.cu` `sampling_penalties_reset_static_cuda_state` | dispatch #1 |
...
| B-3 `s_h_normed` sized from the first model's `d_model` | FIXED — T2 arena take with a capacity field and a reset hook (`engine_scheduler.cpp` `mtp_prenorm_scratch`); `imp.conf.example` `mtp_prenorm_h` reconciled to the code default `true` | `src/runtime/engine_scheduler.cpp` | dispatch #1 |
