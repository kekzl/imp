...
| A1-7 resolved-dispatch dump blind at the gated MoE prefill arms | FIXED (prefill half) - `FUSED_Q6K` gets its producer; `FUSED_Q4K_DP4A` and `GEMMA4_GGML` added, named and recorded at their arms; `DispatchPaths.EveryEnumeratorHasAName` covers them. Still open: no `MoeDecodePath` enum, `run_moe_decode_fast` records nothing | `src/compute/dispatch_paths.{h,cpp}`, `src/exec/executor_forward_moe.cu`, `tests/test_dispatch_record.cpp` | #1916 |
