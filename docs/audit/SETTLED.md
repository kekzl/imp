...
| F2-6 no continuous fuzzing | FIXED (advisory) - `fuzz-nightly` job in `ci.yml` on the existing `17 3 * * *` schedule: `silkeh/clang:18` by digest, the four CPU-only libFuzzer targets 10 min each, corpus in `actions/cache`, crashers uploaded as `fuzz-crashes`; a crasher is a warning, not a red check | `.github/workflows/ci.yml` | dispatch 22 |
