...
| J-1 ARCHITECTURE.md states one KV block size; the resolver picks two | DOC-FIXED - the "Init KV cache" cell names the auto rule (16, raised to 32 when `n_kv_heads <= 4`) and the resolver that applies it | `docs/internals/ARCHITECTURE.md`; `src/runtime/engine_init_resolver.cpp` | #NNNN |
| C-8 ARCHITECTURE.md says non-Gemma-4 MoE disables graph capture | DOC-FIXED - the sentence lists the `GraphDemotionReason` set and says an on-device MoE captures decode graphs like a dense model | `docs/internals/ARCHITECTURE.md`; `src/runtime/graph_eligibility.h` | #NNNN |
| J-2 31 of 223 bound config keys in no example, 8 in no document | FIXED - 31 entries with the defaults and one-line rationale from the config headers; `tools/check_config_keys.py` asserts binder == example in both directions (224/224, selftest 5/5) inside the `docs` gate group | `imp.conf.example`; `tools/check_config_keys.py`; `scripts/ci_static_gates.sh` | #NNNN |
| J-3 FEATURES.md citations point at unrelated code | DOC-FIXED - `src/model/model.cpp:391` (the phi3 alias row) and `Makefile:335` (`COMPARE_URL=`); the existence-only citation gate stays, a content gate is left out | `docs/FEATURES.md` | #NNNN |
| J-4 the README competitive table exists nowhere else | DOC-FIXED - the 2026-08-30 six-model sweep is a `BENCHMARKS.md` section with its PROV block (the Gemma-4 row included); README keeps the extract and its pointer is true | `docs/BENCHMARKS.md` | #NNNN |
| J-5 two L1 performance front doors, one historical and unindexed | FIXED - the old `performance.md` is `docs/archive/performance_2026_05.md` (a record, lint-excluded); its two live paragraphs (bench-mode sizing, healthy-host check) are in `PERF.md` "Reproducing any of this"; the 4 links in `MODELS.md`, `quantization.md`, `KERNELS.md` (x2) target `PERF.md` | `docs/PERF.md`; `docs/archive/performance_2026_05.md` | #NNNN |
| J-6 MODELS.md publishes 268 labelled "CI baseline" | DOC-FIXED - the Qwen3-8B Q8_0 row links the pin instead of restating it, and the header no longer calls the gate model "a different model" | `docs/MODELS.md` | #NNNN |
| J-7 layer is self-declared, so a doc can downgrade its own provenance failures | FIXED - `_layer_for_path` in `scripts/docs_lint.py`: README L0, `docs/*.md` L1, `docs/internals/*.md` L2, `CLAUDE.md` / `AGENTS.md` L3, a mismatch is an error; `vision_gemma4v_spec.md` moved into `docs/internals/` and indexed in `docs/README.md`; paths outside the four homes keep their declaration | `scripts/docs_lint.py`; `docs/internals/vision_gemma4v_spec.md` | #NNNN |
| J-8 the CHANGELOG form rule is not met by 19 of the last 26 entries | FIXED - `tools/check_changelog_form.py` fails any `[Unreleased]` entry over 3 lines (selftest 7/7) in the `docs` gate group; `[Unreleased]` and 0.37.0 compressed to the rule (25 + 6 entries, 0 over); 0.36.0 and older stay as written | `CHANGELOG.md`; `tools/check_changelog_form.py` | #NNNN |
| J-9 the router has no row for the skill that owns the doc gate | DOC-FIXED - one `docs-layers` row | `CLAUDE.md` | #NNNN |
| J-10 the router's env-var list is incomplete (`IMP_WORKER_TIMING`) | FIXED - `diagnostics.worker_timing` bound, in the example, env-seeded at load like `spec_trace` / `jump_trace` / `ppl_dump`; `batching_engine.cpp` reads the key; the router names the six seeded env vars once | `src/core/config/diagnostics.h`; `src/runtime/config.cpp`; `tools/imp-server/batching_engine.cpp` | #NNNN |
| J-11 MODELS.md has no Mixtral row while README and FEATURES mark it green | DOC-FIXED - the FEATURES row names its gate (synthetic 8-expert case in `tests/test_moe_executor.cu` + template family) and says no checkpoint row exists; no Mixtral checkpoint was run | `docs/FEATURES.md` | #NNNN |
| J-12 one measurement, four copies | DOC-FIXED - README and `docs/internals/BENCHMARKING.md` link `PERF.md` (the PROV owner) instead of restating 287.63 / 276.92; the `_thresholds_note` copy in `tests/perf_baseline.json` stays (it explains the threshold to whoever re-pins) | `docs/PERF.md` | #NNNN |
| A2-6 `kernel_resource_baseline.txt` contradicts the FA2 lever record | DOC-FIXED - `attention.h` names the ptxas baseline as the owner (255 regs / 96 B frame on the shipped HD=256 instance, 246 / 0 at Bkv=32, 40 B on the dense 2-CTA instance) and dates the 232 / 226 / 24 B figures to the earlier build; the lever ledger carries the same re-pin; the verdict (two CTAs fit by registers) stands | `src/core/config/attention.h`; `tools/kernel_resource_baseline.txt`; `docs/plans/2026-09-04-lever-ledger-detail.md` | #NNNN |
| A2-7 the smallM header names v2 and the wrong warp split | DOC-FIXED - the block says `smallM_kernel_v1`, 4 producer + 4 consumer warps, thread 0 issues the TMA loads, full / empty barriers per stage, and points at the unrelated dense `_v2` file | `src/compute/gemm_grouped_nvfp4_smallM.cu` | #NNNN |
| C-7 the decode-pipeline diagnostic prints `ssm_ok=1` for the hybrids it refuses | FIXED - the log expression is `!ssm_state_`, the gate it reports | `src/runtime/engine_decode_pipeline.cpp` | #NNNN |
| D-4 LIMITATIONS names MoE atomics as the seed-divergence mechanism | DOC-FIXED - LIMITATIONS and `docs/determinism.md` name the live default-mode sources (cuBLASLt algo selection, the sampler's cross-block reductions, the FP32-compute scatter fallback) and say the default F16 fused scatter has no atomics | `docs/LIMITATIONS.md`; `docs/determinism.md` | #NNNN |
...
| I-9 `TEST_INVENTORY.md` reads as current state | DOC-FIXED - a record header names the four superseded claims (macros 2 125 -> 2 710, hook installed, 61 -> 0 skips, `*Attention*`), the 6 `DISABLED_` tests and the regeneration command; `tests/CLAUDE.md` labels the link a record | `docs/audit/TEST_INVENTORY.md`; `tests/CLAUDE.md` | #NNNN |
| P0-1 SETTLED F-10 "zero times" is stale by one dead include | RE-PINNED in F-10 above - `src/exec/pre_dequant_phase1_fp16_cache.cu` named, removal queued in dispatch #14 | `src/exec/pre_dequant_phase1_fp16_cache.cu` | #NNNN |
| P0-4 `quant -> compute` is 5 edges, not the 1 SETTLED records | RE-PINNED in G above - the 4 `compute/pdl_device.cuh` includes named, the one dispatch edge stands, the move queued in dispatch #14 | `src/compute/pdl_device.cuh` | #NNNN |
