...
| H-8 every third-party pin is a mutable ref (attribution half) | PARTLY - the SageAttention provenance is pinned to a file commit in `THIRD_PARTY_LICENSES.md`; the pin half (SHA pins for the 4 `cmake/imp-deps.cmake` tags, the Dockerfile clones, 9 action refs) has no P3 row and stays open | `THIRD_PARTY_LICENSES.md`; `cmake/imp-deps.cmake` | #NNNN |
