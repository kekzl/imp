...
| A1-7 resolved-dispatch dump blind at the gated MoE prefill arms | FIXED (prefill half) - `FUSED_Q6K` gets its producer; `FUSED_Q4K_DP4A` and `GEMMA4_GGML` added, named and recorded at their arms; `DispatchPaths.EveryEnumeratorHasAName` covers them. Still open: no `MoeDecodePath` enum, `run_moe_decode_fast` records nothing | `src/compute/dispatch_paths.{h,cpp}`, `src/exec/executor_forward_moe.cu`, `tests/test_dispatch_record.cpp` | #NNNN |
| A1-8 `gemm_q4k_fused_moe_prefill` reachable only from a test | FIXED - deleted with `launch_scalar`, `gemm_qk_scalar_moe_prefill_kernel` and `tests/test_gemm_q4k_fused_prefill.cu` (-159 / -338 lines); `gemm_q4k.cu` keeps the live dp4a dense and MoE kernels | `src/compute/gemm_q4k.{cu,h}`, `CMakeLists.txt` | #NNNN |
