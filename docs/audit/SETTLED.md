...
| I-1 = D-3 five model-backed suites run from no target | FIXED — second container in `make test-e2e` with `IMP_TEST_MODEL`/`IMP_TEST_GGUF` = Qwen3-8B-Q8_0 (the model the lock rows name) running `GreedyLockTest.*:DegenerationTest.*:PrefixCacheE2ETest.*:TokenizerCompatTest.*:TensorKindCoverage.*`, a third for the NVFP4-cortecs lock rows. First run: 11 green, the greedy lock RED on the `17 + 25` row for both checkpoints (trajectory moved after token 3 to the continuation llama.cpp gave in the 06-04 verification; answer unchanged); rows regenerated with `IMP_LOCK_PRINT=1`, the mover is unrecorded because nothing ran the lock for 3 months | `Makefile` `test-e2e`, `tests/refs/e2e_greedy_locks.h` | dispatch #4 |
| I-3 `make verify` red by construction | FIXED — `_LANE_FILTER` synced; `scripts/check_lane_filter_copy.sh` (`guard_lane_filter_copy`, unit lane) diffs the two literals, fails on the stale copy (mutation run) | `scripts/check_lane_filter_copy.sh` | dispatch #4 |
| I-7 Makefile filters unguarded against "matches nothing = PASSED" | FIXED — `scripts/check_makefile_filters.sh` (`guard_makefile_filters`, unit lane) resolves every `--gtest_filter` literal against the binary the line names, 17 patterns; it found a LIVE one: `Makefile` ran `test-text --gtest_filter="*Qwen38TokenizerParity*"` for a suite in `test-e2e`, "0 tests, PASSED" on every `make test-gpu` | `scripts/check_makefile_filters.sh` | dispatch #4 |
| I-8 `make test-gpu` sets no model env | DOCUMENTED, not changed — the target's comment states it (every macro, not every checkpoint; the battery is `make test-e2e`); adding three checkpoints to one process hits the VRAM ceiling TEST_INVENTORY section 5 records | `Makefile` `test-gpu` | dispatch #4 |
| H-1 dependency-pin check only in advisory Lint | FIXED — `deps` group in `ci_static_gates.sh` runs `check_dep_pins.sh` offline (sed/grep over two tracked files); Lint keeps `--online` | `scripts/ci_static_gates.sh` | dispatch #4 |
| H-4 pinned baselines executed by nothing; "CI TTFT gate" claim | DOC-FIXED — `docs/BENCHMARKS.md` and the north-star `_ttft_note` now say `make verify-north-star` only, no hook or CI; the targets stay (no new release stage: the 41-day-old pin would red on H-3 first) | `docs/BENCHMARKS.md`, `tests/perf_baseline_north_star.json` | dispatch #4 |
| G-6 function gate stopped at the first column-0 `}` | FIXED — brace-depth walk ignoring comments and string literals (`_braces`), 2 new selftest cases (12/12); the hidden body measures 556 and is allowlisted with a reason (split deferred to dispatch #8, same tiers); no other allow entry moved | `tools/check_function_size.py` | dispatch #4 |
