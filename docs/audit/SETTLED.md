...
| F1-1 GGUF `n_dims > 4` stack write | FIXED — `parse_tensor_infos` fails the reader on `n_dims > 4`; `GgufFaultInjection.FiveDimWordsAreRefusedAtParse` + `NDimsAboveFourIsRefused`. The plain build cannot tell the trees apart (the 5-dim tensor is refused downstream on both, after the write); the ASan lane is the oracle | `src/model/gguf_parse.cpp` | dispatch #2 |
| F1-2 mmproj loader: no bounds check, own parser fork | FIXED — the 180-line fork of `BinaryReader` / value parser / tensor-info loop is deleted; `vision_loader.cpp` uses `gguf_loader_internal.h`, sets `data_base`/`data_limit`, refuses the file on `!gguf_tensor_in_bounds`, clamps the reserve, refuses zero/negative config dims and `block_count > kMaxModelLayers` (the fork also divided by a file-supplied `patch_size`) | `src/vision/vision_loader.cpp` | dispatch #2 |
| F1-5 `block_count` sized arrays before the cap; `swa_layers[i]` unguarded | FIXED — `validate_declared_dimensions` right after `block_count` is read (the expert half stays where it was, still before the expert resizes); `swa_layers[i]` size-guarded. `Gemma4BlockCountAboveLimitIsRefusedBeforeSizing`, `Gemma4ShortSlidingWindowPatternIsNotOverread` | `src/model/gguf_loader.cpp` | dispatch #2 |
| F1-7 `bos_token_id` unbounded to the embedding gather | FIXED — bos/eos outside `[0, tokens.size())` refuse the file; `SpecialTokenIdOutsideVocabIsRefused` fails with the check removed | `src/model/gguf_loader.cpp` | dispatch #2 |
| F1-11 truncated shard: `size() - stensor_count` wraps | FIXED — the shard path got the fourth `failed()` check; no split-file fixture, so no test | `src/model/gguf_loader.cpp` | dispatch #2 |
| F1-12 fuzz coverage 2 of 10 parsers | PARTLY — `fuzz_gguf` (`load_gguf`) and `fuzz_mmproj` (`vision_gguf_probe`, the dry pass) in `tests/test_fuzz_corpus.cpp` and the `Sanitizers` job: 8 of 10. Open: `config.json`, Jinja, SentencePiece, warm cache, LoRA | `fuzz/fuzz_gguf.cpp`, `fuzz/fuzz_mmproj.cpp` | dispatch #2 |
