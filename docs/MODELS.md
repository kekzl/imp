...
The decode column is a per-model figure, not a gate; each comes from the sweep named below. The gated number CI defends (Qwen3-8B Q8_0 tg128) lives in [`PERF.md`](PERF.md); its row here links it instead of restating it.
...
|---|---:|---:|---:|---|
| [nomic-embed-text-v1.5](https://huggingface.co/nomic-ai/nomic-embed-text-v1.5-GGUF) | Q8_0 | 0.2 GB | embeddings (`/v1/embeddings`, HF-oracle cos ≥ 0.999), bidirectional, no KV, mean-pooled | GGUF |
...
| [Phi-4-reasoning-plus](https://huggingface.co/nvidia/Phi-4-reasoning-plus-NVFP4) | NVFP4 | 9.0 GB | 157 | SafeTensors (nvidia), fused projections. Phi-4 is an alias onto the LLaMA loader path (`src/model/model.cpp:328`), not a separate loader; no dedicated gate (#1680) |
...
| [Qwen3.8-27B](https://huggingface.co/Qwen/Qwen3.8-27B) | NVFP4 | 17 GB | 90 | SafeTensors, dense-GDN 27B (64 layers: 16 attn + 48 GDN) and multimodal. The text config is field-identical to Qwen3.6-27B above. **No published NVFP4 export runs on this card**: they are mixed-precision with FP8 attention and `sm_120` has no FP8 GEMM. A ready export is published: [kekzle/Qwen3.8-27B-NVFP4-vllm](https://huggingface.co/kekzle/Qwen3.8-27B-NVFP4-vllm) (`imp-quantize --format vllm`; the same directory loads here and in vLLM, verified on 0.27.1 and 0.28.0). Or quantize the BF16 release yourself (`imp-quantize`, 51.8 → 19.2 GiB). Teacher-forced PPL 4.62 on `ppl_corpus_45k.txt` against 7.55 for Qwen3.6-27B on the same corpus. The FP8 release works as a source too and is a third of the download (28.8 GiB), for 0.24% more perplexity; it does **not** run unquantized, its weights need 26 952 MiB and the upload aborts. **Give it room to think**: in a long conversation it spends its token budget on reasoning first, so `max_tokens` below ~400 can return an empty `content` (74-turn session: several empty replies at 260, 74/74 clean at 600; the same session degenerates on vLLM too). **Its embedded `mtp.*` head is taken automatically on a single-stream run** (`speculative.mtp_k=-1` auto): 95.8 -> 141.6 tok/s on a thinking prompt, 2026-08-29. A server taking concurrent requests declines it, says so in `/health`, and now names the refusal in every affected response as well (`imp_spec_declined = "mtp_head_not_loaded"`, in `usage.completion_tokens_details` on chat, top-level `usage` on `/v1/messages`, `usage.output_tokens_details` on `/v1/responses`); `--set speculative.mtp_k=2 --set speculative.ngram=false` forces it at the server, and a single request can pick its own depth with `"speculative": {"mtp_k": N}` up to the armed one. Pinning `kv_cache.dtype=fp8` here also costs the fast decode kernels, which are head_dim-128 only against this model's 256 - the scalar path serves it and now says so at init. **Long-context posture** (2026-08-29): do not pin the KV dtype - `auto` resolves NVFP4 here (16384 B/tok against 32768 for FP8), worth +45.6% context; pinning `runtime.max_batch_size=8` frees recurrent state into the pool (222 368 pool tokens, the binding limit moves to the `max_seq_len` auto cap of 131 072). NIAH 5/5 at 126 908 tokens, and since #1817 there is no decode trade left: NVFP4 74.1 vs FP8 72.4 tok/s at 77k context (2026-08-30, forced-equal emitted tokens, 0.3% spread). Before that fix FP8 led by 13.5%, so a pin copied from an older deployment is now wrong on both axes. **NVFP4 KV capacity**: a 27B model holds 126 432 tokens of context against 86 848 for 8-bit KV; at a 77k prompt it decodes 74.3 tok/s, or 100.2 with sparse decode on [PROV: commit=3921547d date=2026-08-30 hw=RTX5090 quant=NVFP4-kv cuda=13.3 cmd=`imp-server --max-batch 8` n=3x3 note=sparse arm adds `--set attention.sparse_topk_tokens=8192`]. **Sparse decode** (`attention.sparse_topk_tokens`, default off) works on this KV dtype since #1818: at 77k it reads 100.2 tok/s against 74.3 dense. The retrieval cost was a property of the min/max corner bound, not of sparse decode: with the default mean+std page score this model is NIAH 10/10 at a 4096 budget and at 8192, against 2/10 and 7/10 for the corner bound on the same binary, wall time neutral (2026-09-12, 5 depths x 81 908 / 126 908 tokens). The older corner-bound figures (8/10 at 8192, 9/10 at 32768, 5/10 at 4096) apply only with `attention.sparse_score_meanstd=false`. It holds 10/10 down to a 1024-token budget, but NIAH is retrieval only and the budget is not a speed lever here (1024 against 8192 is inside the run-to-run spread, 2026-09-12), so configure it for headroom, not for throughput. Budgets mean what they say since #1819; before it this geometry silently got 2x. Ledger: [plans/2026-08-29-qwen38-long-context-posture.md](plans/2026-08-29-qwen38-long-context-posture.md). See [TROUBLESHOOTING.md](TROUBLESHOOTING.md) |
...
Several images per request are supported on this tower (repeat `--image`, or several `image_url` parts), read in prompt order.
...
and serves the language model alone. Gemma-4 *does* see images, but only through the GGUF + `--mmproj` pair in the table, not from its NVFP4 SafeTensors export.

Since #1198 this is visible from the client, not only the server log: a request carrying `image_url` parts the loaded model cannot use is refused with **400 `vision_unavailable`** instead of being answered from the text alone (a fluent description of a picture the model never received is indistinguishable from a real one).
...
Image tokens cost VRAM in the encoder workspace, sized by `runtime.vision_max_patches` (default 4096 ≈ 1024x1024). Larger images are scaled down to fit that budget rather than refused.
