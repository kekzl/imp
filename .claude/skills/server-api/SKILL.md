...
- **A byte-level regex engine already exists**: `RegexNfa` in `src/compute/json_schema.h` (compile / start_set / step / accepts), used for JSON-Schema `pattern`. It handles literals, classes with negation, `\d\w\s`, alternation, grouping and `{n,m}`. Before writing a grammar engine for anything, check whether this covers it — a from-scratch DFA was built and discarded in 2026-07 after measuring identical behaviour.
