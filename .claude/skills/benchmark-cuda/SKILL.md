...
7. **A pinned number is only comparable to a run with the SAME flags — check what the flags measure before calling anything a regression.** Two cases, both cost a session:
   - **Prefill pins from before #1061 measured prefix-cache hits.** Prefix caching went default-on in #758 and `--bench` repeats one prompt, so reps 2..N partly hit the cache. #1061 disables prefix caching for one-shot CLI runs, which dropped pp512 on Qwen3-8B from 14515 to **12407** with no kernel change. If you need the old behaviour for an A/B: `--set server.prefix_cache=true` reproduces it.
   - **`tg128_at_ctx_2048` needs `--bench-pp 2048`**, or decode does not actually run at ctx=2048. The same build reads **169.00** with `--bench-pp 16`, **164.83** with `--bench-pp 512`, **149.47** with 2048. The first two look like double-digit north-star gains and are just shorter contexts.
   Also: `verify-fast` runs fewer trials than `gen-perf-baseline`, so it reads lower on the same build — a `verify-fast` "regression" of a few percent is often methodology, not the code. Confirm with a cold-median run before believing it.
8. **No-graphs nsys/ncu time shares OVERSTATE tiny-kernel classes — validate any launch/latency-class lever with a graphs-ON e2e A/B BEFORE building it.** The roofline pipeline profiles with `--no-cuda-graphs`; under the shipped graphs+PDL decode loop the tiny launches largely overlap away. Measured 2026-07-13 (Qwen3-30B-A3B): the sum of no-graphs per-kernel times is ~4.6 ms/step vs the real graphs-ON step of ~2.55 ms — 44% of the no-graphs window is already hidden. Concretely: fusing gemv_gate_fp32+topk_gating (6.9% no-graphs window share, 2 launches/layer eliminated, bit-identical outputs) moved e2e by **0%**; capping decode split-K (reduce pass "5.8 µs at 40 GB/s") REGRESSED −21 to −35% because the splits' SM-fill matters more than the reduce. The Amdahl lever list in the roofline report is only trustworthy for classes that hold real bytes/critical-path time (big GEMVs, data-dependent chains) — not for grid-(1,1,1) launch-latency classes (moe_routing, rmsnorm, rope, kv_write, elementwise).
...
- **`tests/perf_baseline.json`** is the canonical CI gate. Refresh ONLY when a change *intentionally* moves perf: `make gen-perf-baseline`, on a healthy-host day (STOP #4), and say so in the PR. **The gate measures spec-OFF decode** (`--set speculative.ngram=false`, since 2026-07-15): with speculation ON the self-repetitive bench prompt (~99.9% accept) measures the batched spec-verify GEMMs, which are restart-volatile like cuBLAS prefill (11% swing on healthy clocks) — ungateable at 3%. Current pins (re-pinned 2026-07-26, PR #1079): Q8 `tg128 = 287.19`, `pp512 = 12407`, `pp4096 = 15325`; north-star 14B `tg128 = 165.10`, `tg128_at_ctx_2048 = 149.47`. **Prefill pins before that date are NOT comparable** — see STOP #7. Historic lesson: a 286.4 baseline (2026-06-07) was sampled on a documented **peak day** — its 3% gate threshold sat INSIDE the normal range, so it failed on ordinary days; it was corrected down in #697. A small gate failure can be host drift, not a regression; sample clocks before concluding.

When a gate fails, rule out the cheap causes in this order before bisecting: (1) is anything else on the GPU (`nvidia-smi --query-compute-apps=pid`) — a forgotten server container reads −12%; (2) does the diff touch the measured code at all (`git diff --stat main -- src/ include/ tools/imp-cli/` — empty means a decode regression is impossible); (3) does a cold-median run reproduce the verify-fast number. Only then bisect.
